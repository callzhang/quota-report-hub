...
- `POST /api/auth/fetch-best`
  ...
  - if the requester owns an invalidated auth, returns that auth as `repair_auth`; the guard installs the owner handback once and pins it until an explicit re-login changes the credential, rather than treating its expected 401 as a request for another shared account
