...
The Hub has an independent `token-usage.html` page for authenticated team members. It defaults to the previous seven days, hourly buckets, grouping by Hub user, the Codex provider, and the Total counter. Queries can filter by time, Hub user, provider (`codex` or `claude`), model account, and the raw provider model name, then group by Hub user, provider, model account, or model. Per-user totals are shown in Breakdown rather than a duplicate side panel. Every figure is also priced in dollars: Spend leads the summary cards, is its own Breakdown column, and can be selected as the metric, in which case it ranks the per-user bars and the table. The price list is `lib/model-tiers.js`, the same one the fetch gate spends, so a model the pool does not pay for — an own-key DeepSeek, a self-hosted Qwen or MiniMax — shows its real tokens and $0 of demand. The page uses only `GET /api/token-usage-query` and caches each exact query for five minutes per login session.
...
- Codex full-RT uploads are custody handoffs: the Hub does not refresh the uploaded RT, and the local guard strips its disk copy. When the local account or RT/AT generation changes, the guard completes the previous account's handoff without stopping the app-server and keeps the new account pending. If the generation did not change, it waits for a fresh explicit idle snapshot bound to the current app-server PID, then uses `codex app-server daemon restart` or, only after an unmanaged-daemon refusal, sends `SIGTERM` to that exact listener PID and confirms it exited. Missing or ambiguous PID identity is fail-closed; it never stops an active or unknown app-server and never launches `codex login`.
...
- The local guard treats a changed local Codex account or auth generation as proof that the app-server has moved to the new RT/AT generation: it unlocks the previous account's pending Hub handoff without restarting, then uploads the new account as pending. Otherwise, it requires fresh idle activity evidence before restarting or retiring the app-server; missing, stale, malformed, active, or unknown evidence is fail-closed.
- Claude rows can be refreshed by the cloud worker for direct Claude subscriptions, or by stable local client reports when Claude is running in an environment that the worker cannot replay reliably. Hub-bound local Claude quota comes only from the OAuth usage API or a cache written by the exact same access-token fingerprint; changing tokens bypasses old windows and old backoff. The statusline snapshot remains available for local terminal display, but is not quota evidence because its payload does not identify the credential held by that session.
...
- A Codex full-RT upload may set `defer_codex_refresh:true`; the Hub stores that account as `refresh_handoff_state=pending`, strips the local RT after read-back, and blocks upload verification, worker probes, `refresh_current`, and id-token refresh until the handoff completes. A changed local RT/AT generation completes the previous account before the new account is uploaded; the new account remains pending until its own generation changes or a safe idle handoff is confirmed.
...
- Claude quota is probed in the worker by running `claude -p /usage` against the restored CLI state and parsing the windows it prints. No inference turn is spent and no session is registered on the pooled account. The previous approach — driving the interactive UI and scraping its statusline snapshot — is kept as plan B behind `--mode tui`.
...
- Plan B (`--mode tui`) uses a short statusline refresh interval during probing so the snapshot is emitted before the worker timeout expires; the default usage-mode probe needs no snapshot at all.
...
- if the authenticated uploader has an invalidated Codex auth, the server returns that uploader's own account as `repair_auth` before considering any healthy shared replacement; the local guard installs the handback once, marks it `repair_auth_from_auth_pool`, and holds it stable at `owner_relogin_required` until an explicit login changes the credential digest. While repair is pending it is neither re-uploaded nor replaced, so its expected 401 cannot start an account-switch loop
