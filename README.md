...
- Codex full-RT uploads are custody handoffs: the Hub does not refresh the uploaded RT, and the local guard strips its disk copy. When the local account or RT/AT generation changes, the guard completes the previous account's handoff without stopping the app-server and keeps the new account pending. If the generation did not change, it waits for a fresh explicit idle snapshot, then uses `codex app-server daemon restart` or, only after an unmanaged-daemon refusal, sends `SIGTERM` to exact current-user/current-home listener PIDs and confirms they exited. It never stops an active or unknown app-server and never launches `codex login`.
