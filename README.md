...
- `WAITING FOR NEW QUOTA` means the prior window reset and no post-reset snapshot has arrived; `QUOTA UNKNOWN` means current quota evidence is missing, stale, partial, or from a failed probe; `UNAVAILABLE` means the credential itself is rejected, invalidated, expired without a stored refresh token, or ineligible. The pool stores and exposes only a boolean refresh-capability marker, never the refresh token itself
