...
- **Claude store ranking is capability-first.** `claude_credential_rank` asks the provider whether a
  store's token may do inference (cached per token per run) and ranks that above a real refresh
  token, which used to dominate. Measured 2026-09-17 on this machine: a terminal login wrote a
  working credential to the keychain (the 06:39 guard cycle reported real quota), Claude.app restored
  its own grant — refused for inference — into `tokenCacheV2` with a real RT, and by 06:55 the guard
  had picked the app's token, uploaded it, and stripped it over the login. An unreachable provider
  returns *unknown* and demotes nothing, so an offline machine keeps the old ordering.
- **A strip removes a refresh token; it never installs an access token.**
  `strip_claude_refresh_token_from_all_stores` now writes each store's **own** credential back minus
  its RT. It used to write the picked store's credential into every store, which is the mechanism
  that carried the app's refused token into the keychain in that same 06:55 cycle. The cache strip
  was already per-entry and in place.
...
- **Claude quota provenance**: Hub-bound quota comes only from live `/api/oauth/usage` measured
  ...
  polling backoff. The statusline hook remains a useful local terminal display, but its payload has
  no credential identity and a live session can retain an old token after the guard installs a new
  one, so its snapshot is not quota evidence for the Hub. On 2026-09-14 an unbound usage cache let
  11%, 0%, and 41% weekly snapshots from different token epochs oscillate under
  `claude-leizhang0121@gmail.com`; expiry-only cache validation could not detect the misattribution.
...
- An account with `exhausted_until` in the future renders as `low_quota` / `usage_limit_exhausted`, feeding the reset time into its next-transition candidates (the earliest of that reset or the report's staleness boundary wins) — measured-drained, not unknown. This check deliberately ignores report staleness: `exhausted_until` is a provider-issued absolute deadline, not a decaying measurement, so it stays authoritative even once the surrounding report has gone stale. The dashboard reads the deadline off the item (this state deliberately ships no `current_quota` — a snapshot would suppress the row summary): the row shows the reset countdown in the quota windows' `formatResetCountdown` style, the popover adds a "Usage limit" section with the absolute reset moment, and snapshot windows whose own reset is still ahead are shown live rather than branded "Historical".
...
- `low_quota`: all required evidence is current, but a required window is below the rotation threshold (5-hour below 20%, weekly below 5%) — except the exhaustion path above, which reaches `low_quota` without that currency requirement. Claude always requires both windows. Codex requires the weekly window and, only while the report's 5-hour window is *current*, that one too: Plus and Team accounts meter a 5-hour window (measured 2026-09-10, they report it every cycle), Pro does not, and the merge carries an old window forward past its reset for display, so presence alone would hold a Pro account to a months-old zombie. "Current" is the same test selection applies — an expired window counts as missing, and so does a reading older than the window it describes ([§10](#10-selection-algorithm)) — so the dashboard and `fetch-best` never disagree about an account. The display layer shows every window the merged report carries; it used to null the codex 5-hour window outright (2026-08-07, "weekly-only"), which listed a Plus account as available while its own probe read 5h=2%. A window whose reading has outlived its own length is dropped in the same pass, on the same test the decision layers use, so it renders as `n/a` — the account has no such window — rather than as a live number or as `quota_window_expired`, whose wording ("waiting for fresh quota") promises a refresh that a tier metering no such window will never send.
- `available`: all required windows are current and meet the same thresholds used by auth selection.

**Where a row is listed follows this state, not a separate rule.** `authPoolStatusPayload` annotates every row once and then partitions on the derived state: anything `unavailable` leaves the active table for **Invalidated Accounts** immediately, so the Availability column and the table a row sits in can never disagree. There is no grace period — an account nobody can use is not an active pool entry, and the 48-hour delay this replaced made the active table overstate what the pool actually held. The only remaining clock is retirement: a row dead longer than 14 days (dated by `deadForMs` — the later of the refresh token's death and the access token's expiry) drops off **both** lists, since excluding it from only the invalidated one would hand it back to the active table. An unavailable row whose death cannot be dated stays listed; no date is not evidence of age. The wire keys stay `archived_invalidated_items` / `archived_invalidated_count` because deployed guards read them (`quota_guard.py`), and renaming them is a client-visible protocol change ([§9b](#9b-the-premium-share-gate-libpremium-ratiojs)).
...
`reset_at`), a hard invalidation, or a valid `exhausted_until` timestamp. Only the weekly window is
...
### 7.3 Probe mechanics
- A quota read fails on two conditions, reported separately: `codex returned no rate limits` and `codex returned no quota window of a duration this hub models`. The second records `usage_summary.observed_windows` — the `window_minutes`/`used_percent` the response did offer — because `codex_window_key_for_minutes` places only 300 and 10080 and anything else is dropped. Numbers only, never a token or an id. **A missing `info` is not one of the conditions**: it carries `model_context_window` and token counts, nothing about quota, and gating on it discarded complete correct windows — `starbench@stardust.ai` reported six such failures on 2026-09-07/08 and the one measured at 2026-09-08T09:57:45Z carried `primary {window_minutes: 10080, used_percent: 0}` on the plan bucket. `model_context_window` is null when `info` is absent.
...
- **Claude** (`probe_claude_auth_blob.py`): materializes the blob into a temp `HOME` and runs `claude -p /usage`, parsing the plaintext windows with `parse_usage_windows` (~1.5s, no inference turn, no session registered on the pooled account). The temp `HOME` gets `remoteControlAtStartup: false` / `autoUploadSessions: false`, and only whitelisted `~/.claude.json` keys (`CLI_STATE_ALLOWED_KEYS`) are restored. **No `refresh_capture`** (Claude never self-refreshes here); `model_context_window` is always null. A dead access token is invisible in that output (exit 0, header only, no windows), so the worker gates on `probeClaudeAccessToken`'s **401** and maps it to `claude auth invalid (authentication_error)`. **Plan B** (`--mode tui` / `PROBE_CLAUDE_MODE=tui`) keeps the original `pexpect` UI driver + statusline scrape.
...
**Expired windows count as missing, and so do readings that have outlived their own window.** Every selection read of a window (`windowRemainingPercent`) drops a window whose `reset_at` is at or before the report's own `reported_at`, and — `windowEvidenceLive`, `lib/report-freshness.js` — one whose `captured_at` is more than the window's own length behind that report. The second rule exists because the first trusts a provider timestamp that is not always the provider's: a client on 2.1.0 that hit "workspace out of credits" fabricated a 5-hour window and stamped it with the *weekly* reset time, and since a Pro account reports no 5-hour window at all, nothing ever replaced it. `leizhang0121@gmail.com` therefore carried a 2026-09-06 "5h 0%" dated 2026-09-12 and sat out of rotation for four and a half days with a 98% weekly window (measured 2026-09-10). Judging a reading by its own age needs no timestamp from the provider and is scale-correct — five-hour evidence dies in five hours, weekly evidence lasts a week. A window with no `captured_at` on record cannot be judged this way and keeps its value. The original reason for the first rule: the provider has rolled that window over, so its stored numbers describe a spent period. The merge layer deliberately carries such windows forward for the dashboard's stale-evidence display ([§6.4](#64-availability-read-model-and-lazy-history)), so expiry is enforced here, at the decision boundary, not at merge. The forcing case (2026-09-03, codex `leizhang0121`): a Pro account's probes stopped reporting a 5h window on 08-29 — Pro no longer meters one — and the last synthesized "5h 0%" (a weekly-exhaustion pair from the client's `zero_remaining_window`) was carried five days past its reset, failing the 5h threshold while the live weekly window sat at 96%. A window without `reset_at` cannot be judged and keeps its value.
...
- **`pool_health_snapshots`** — one row per source per worker run: `total, ok_count, hard_dead_count, other_err_count, skipped_count, central_refresh_{attempted,ok,rejected}`. `skipped_count` is the entries the worker deliberately did not probe because their Codex refresh handoff is pending (§9 item 7): they are neither healthy nor unhealthy, so they sit in their own bucket rather than in `other_err_count`. Before the split they read as errors — on 2026-09-25 that showed codex as 2/19 healthy when 13 of the 17 non-dead accounts were merely unprobed and their last reports were ok. A skipped account that was already invalidated is invisible in `hard_dead_count` until its handoff completes.
- **Account availability** (`index.html`): one primary lifecycle state per account. Detailed probe/token/refresh evidence and the lazy 24-hour quota chart are secondary diagnostics, not peer status lines. Its 5-hour and weekly series keep their teal and orange identities in both line/point/legend; historical evidence is faded rather than recolored gray so the two windows remain distinguishable.
