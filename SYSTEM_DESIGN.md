...
### 7.3 Probe mechanics
- A quota read can fail at three different points and they are reported separately: `codex returned no rate limits`, `codex returned no quota window of a duration this hub models`, and `codex returned quota but no token usage info` (ordered by closeness to quota — `info` carries the context window, not quota). The middle one also records `usage_summary.observed_windows` — the `window_minutes`/`used_percent` the response did offer. `codex_window_key_for_minutes` places only 300 and 10080; anything else was previously dropped with a message that named none of it, which is how `starbench@stardust.ai` reported the same failure six times on 2026-09-07/08 with nothing to diagnose from. Numbers only, never a token or an id.
