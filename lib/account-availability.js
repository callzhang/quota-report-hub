...
import { MAX_REPORT_AGE_SECONDS, reportIsFresh, windowEvidenceLive } from "./report-freshness.js";
...
function sourceWindowNames(item, generatedAtMs) {
  if (String(item?.source || "").toLowerCase() !== "codex") {
    return CLAUDE_WINDOWS;
  }
  // Codex tiers differ: Plus and Team accounts meter a 5h window, Pro does not. Judge each account
  // by the windows its own report carries — requiring 5h unconditionally would park windowless
  // accounts in quota_unknown forever. Presence alone is not enough either: the merge carries a
  // window forward past its reset for the stale-evidence display, and a Pro account whose last
  // real 5h reading is months old must not read as waiting_for_new_quota on it. Only a window that
  // is current constrains, which is the same rule selection applies (an expired window counts as
  // missing, lib/auth-pool.js), so the dashboard and fetch-best never disagree about an account.
  return isCurrentWindow(item?.display_windows?.["5h"], generatedAtMs, "5h") ? CLAUDE_WINDOWS : ["1week"];
}
...
function isCurrentWindow(window, generatedAtMs, windowName) {
  const resetMs = resetAtMs(window);
  if (remainingPercent(window) === null || window?.reset_unavailable_reason) {
    return false;
  }
  // Mirrors selection (lib/auth-pool.js): a reading that has outlived its own window is spent, and
  // is the one way a bad reset_at survives the check below.
  if (!windowEvidenceLive(window, windowName, generatedAtMs)) {
    return false;
  }
  if (resetMs === null) {
...
function deriveAccountAvailabilityState(item, generatedAt) {
  ...
  const names = sourceWindowNames(item, generatedAtMs);
  ...
  const allCurrent = names.every((name) => isCurrentWindow(windows[name], generatedAtMs, name));
