...
function deriveCodexAuthPoolEntry(authJsonText, reporter = {}) {
  ...
  return {
    ...
    auth_expires_at: identity?.exp ? new Date(identity.exp * 1000).toISOString() : null,
...
function reportIsFresh(report, options = {}) {
  ...
  const maxAgeSeconds = Number(options.max_report_age_seconds ?? 3600);
...
export function pickBestAuthPoolCandidate(reports, authPoolEntries, options = {}) {
  ...
  const maxFiveHour = eligibleCandidates.reduce(
    (max, report) => Math.max(max, windowRemainingPercent(report, "5h")),
    -1
  );
  const fairSpreadSlack = Number(options.fair_spread_quota_slack_percent ?? 20);
  const minimumFiveHour = Number.isFinite(fairSpreadSlack) && fairSpreadSlack > 0
    ? maxFiveHour - fairSpreadSlack
    : maxFiveHour;

  const candidates = eligibleCandidates
    .filter((report) => windowRemainingPercent(report, "5h") >= minimumFiveHour)
    .sort((left, right) => {
      const servedDelta = recentServedCount(left, options) - recentServedCount(right, options);
      if (servedDelta !== 0) {
        return servedDelta;
      }
