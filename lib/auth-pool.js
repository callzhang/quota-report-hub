...
function candidateQuotaWeight(report) {
  ...
  const weight = Math.min(fiveHour, weekly);
...
function projectedWeightedLoad(report, options = {}) {
  ...
  const offset = selectionKey
    ? deterministicUnitInterval(`${selectionKey}:${report.source}:${report.account_id}`)
    : 1;
  return (recentServedCount(report, options) + offset) / weight;
}
