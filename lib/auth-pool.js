...
import { reportIsFresh, windowEvidenceLive } from "./report-freshness.js";
...
function windowRemainingPercent(report, key) {
  ...
  // The same judgement one step further: a reading older than the window it describes is spent
  // whatever its reset_at claims, and a fabricated reset_at is exactly how one survives
  // (windowEvidenceLive). Age is measured against the report, not a clock, so this stays pure.
  if (!windowEvidenceLive(window, key, reportedMs)) {
    return -1;
  }
  return Number(value);
}
...
function isPoolIneligible(report, entry = null) {
  if (isHardInvalidation(report)) {
    return true;
  }
  // The plan is read from the CREDENTIAL (auth_pool_entries.plan_name, the id_token claim), never
  // from the probe payload. A report's plan_name comes from the rate-limit response, which upstream
  // can disagree with the token that made the request (openai/codex#29243, AUTH_TOKENS §6.6) -- and
  // a single transient "Free" is enough to evict a working account here. Observed 2026-09-12:
  // professional@stardust.ai reported Free exactly once, between two Pro readings. The dashboard's
  // three Free checks already read the entry (reports.js builds its rows with `plan_name:
  // entry.plan_name`); this was the one site still trusting the probe. Fall back to the report only
  // when the credential is gone, where there is nothing better to consult.
  const planName = entry?.plan_name ?? report?.plan_name;
...
export function pickBestAuthPoolCandidate(reports, authPoolEntries, options = {}) {
  ...
  const eligibleCandidates = reports
    ...
    .filter((report) => !isPoolIneligible(report, entryByAccount.get(report.account_id)))
...
export function shouldReplaceAuthPoolEntry(existingEntry, incomingEntry, options = {}) {
  if (!existingEntry) {
    return true;
  }
  ...
  // A standing central-refresh rejection is the worker having presented THIS pooled refresh token to
  // the provider and been refused, so the stored credential can never be renewed again: its access
  // token is a countdown to a hard death with nothing behind it. An upload carrying a real refresh
  // token can be renewed indefinitely, so it wins outright and the expiry comparison below never
  // runs. Without this, the comparison keeps whichever access token merely lives longer -- which for
  // Claude is the only thing auth_last_refresh records -- and on 2026-09-11 that kept a credential
  // the worker had been refused on for 48 straight probes over one minted minutes earlier by a fresh
  // login. The refused upload was then stripped locally as designed, so the account's only working
  // refresh token ceased to exist anywhere.
  //
  // `incomingHasRealRefreshToken` is the caller's `isStrippedRefreshToken` verdict over the actual
  // blob, NOT `incomingEntry.has_refresh_token`: the hub placeholder is a non-empty string, so that
  // flag reads true for an access-token-only upload and would let a stripped blob displace the
  // incumbent -- precisely the poisoning the strip guard exists to prevent. A rejected incumbent
  // still beats an access-token-only upload: nothing can renew either, and the incumbent is at
  // least what the pool already serves.
  if (options.existingRefreshRejected && options.incomingHasRealRefreshToken === true) {
    return true;
  }
