...
// token it ran on. The probe need not have produced usable quota: a meter in its report means the
// provider authenticated the token and returned rate limits for it, which is what "working" means
// here. hr@stardust.ai on 2026-09-22 answered "workspace out of credits" -- an error for rotation,
// and still proof the credential is live. A token the provider refused yields no meter at all.
// It is a witness only for the token being uploaded: if the CLI rotated the file between probe and
// upload the fingerprints differ and the upload proves nothing.
export function bundledProbeWitnessesAccessToken({ quotaPayload, uploadedAccessTokenFingerprint }) {
  return Boolean(
    quotaPayload?.usage_summary?.meter?.limit_id &&
