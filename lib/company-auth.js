import crypto from "node:crypto";

const DEFAULT_ALLOWED_DOMAIN = "stardust.ai";

export function allowedDomain() {
  return (process.env.AUTH_ALLOWED_EMAIL_DOMAIN || DEFAULT_ALLOWED_DOMAIN).toLowerCase();
}

export function normalizeEmail(email) {
  return String(email || "").trim().toLowerCase();
}

export function companyEmailAllowed(email) {
  const normalized = normalizeEmail(email);
  return normalized.endsWith(`@${allowedDomain()}`);
}

// Only consulted by the one-time auth_admins seed (lib/db.js): admins live in the database now,
// and ADMIN_EMAIL is just what a pre-database deployment carries into it.
export function adminEmails() {
  return String(process.env.ADMIN_EMAIL || "")
    .split(",")
    .map((value) => normalizeEmail(value))
    .filter(Boolean);
}

export function authMailConfigured() {
  return Boolean(
    process.env.MAILGUN_API_KEY &&
    process.env.MAILGUN_DOMAIN &&
    process.env.MAILGUN_FROM
  );
}

export function authPoolConfigured() {
  return Boolean(
    process.env.TURSO_DATABASE_URL &&
    process.env.TURSO_AUTH_TOKEN &&
    process.env.AUTH_POOL_ENCRYPTION_KEY
  );
}

export function opaqueToken(prefix) {
  return `${prefix}_${crypto.randomBytes(24).toString("base64url")}`;
}

export function tokenHash(token) {
  return crypto.createHash("sha256").update(String(token)).digest("hex");
}

function tokenIssueKey() {
  const raw = process.env.TOKEN_ISSUE_KEY;
  if (!raw) {
    throw new Error("TOKEN_ISSUE_KEY is not configured");
  }
  return raw;
}

const DASHBOARD_REVISION_TOKEN_TTL_MS = 12 * 60 * 60 * 1000;

export function signDashboardRevisionToken(email, issuedAt = Date.now()) {
  const payloadB64 = Buffer.from(JSON.stringify({
    e: normalizeEmail(email),
    s: "dashboard_revision",
    exp: issuedAt + DASHBOARD_REVISION_TOKEN_TTL_MS,
  }), "utf8").toString("base64url");
  const hmac = crypto
    .createHmac("sha256", tokenIssueKey())
    .update(`dashboard-revision:${payloadB64}`)
    .digest("base64url");
  return `qrr.${payloadB64}.${hmac}`;
}

export function verifyDashboardRevisionToken(rawToken, now = Date.now()) {
  const parts = String(rawToken || "").split(".");
  if (parts.length !== 3 || parts[0] !== "qrr" || !parts[1] || !parts[2]) {
    return null;
  }
  let expectedHmac;
  try {
    expectedHmac = crypto
      .createHmac("sha256", tokenIssueKey())
      .update(`dashboard-revision:${parts[1]}`)
      .digest("base64url");
  } catch {
    return null;
  }
  const provided = Buffer.from(parts[2]);
  const expected = Buffer.from(expectedHmac);
  if (provided.length !== expected.length || !crypto.timingSafeEqual(provided, expected)) {
    return null;
  }
  try {
    const payload = JSON.parse(Buffer.from(parts[1], "base64url").toString("utf8"));
    if (
      payload.s !== "dashboard_revision" ||
      typeof payload.e !== "string" ||
      !Number.isFinite(payload.exp) ||
      payload.exp <= now
    ) {
      return null;
    }
    return { email: normalizeEmail(payload.e) };
  } catch {
    return null;
  }
}

export function signTokenPayload(email) {
  const nonce = crypto.randomBytes(18).toString("base64url");
  const iat = new Date().toISOString();
  const payload = JSON.stringify({ e: email, n: nonce, t: iat });
  const payloadB64 = Buffer.from(payload, "utf8").toString("base64url");
  const hmac = crypto
    .createHmac("sha256", tokenIssueKey())
    .update(payloadB64)
    .digest("base64url");
  return {
    token: `qrp.${payloadB64}.${hmac}`,
    email,
    created_at: iat,
  };
}

export function verifyTokenPayload(rawToken) {
  if (!rawToken || typeof rawToken !== "string") {
    return null;
  }
  const parts = rawToken.split(".");
  if (parts.length !== 3 || parts[0] !== "qrp") {
    return null;
  }
  const payloadB64 = parts[1];
  const hmac = parts[2];
  if (!payloadB64 || !hmac) {
    return null;
  }
  let expectedHmac;
  try {
    expectedHmac = crypto
      .createHmac("sha256", tokenIssueKey())
      .update(payloadB64)
      .digest("base64url");
  } catch {
    return null;
  }
  if (!crypto.timingSafeEqual(Buffer.from(hmac), Buffer.from(expectedHmac))) {
    return null;
  }
  try {
    const payload = JSON.parse(
      Buffer.from(payloadB64, "base64url").toString("utf8")
    );
    if (!payload.e || typeof payload.e !== "string") {
      return null;
    }
    return {
      email: normalizeEmail(payload.e),
      created_at: payload.t || null,
    };
  } catch {
    return null;
  }
}

export function bearerTokenFromHeaders(headers = {}) {
  const header =
    headers.authorization ||
    headers.Authorization ||
    "";
  return String(header).replace(/^Bearer\s+/i, "").trim();
}

async function sendMailgunEmail({ to, subject, text, html }) {
  const apiKey = process.env.MAILGUN_API_KEY;
  const domain = process.env.MAILGUN_DOMAIN;
  const from = process.env.MAILGUN_FROM;
  const baseUrl = process.env.MAILGUN_BASE_URL || "https://api.mailgun.net";

  const form = new FormData();
  form.set("from", from);
  form.set("to", to);
  form.set("subject", subject);
  form.set("text", text);
  form.set("html", html);

  const response = await fetch(`${baseUrl}/v3/${domain}/messages`, {
    method: "POST",
    headers: {
      Authorization: `Basic ${Buffer.from(`api:${apiKey}`).toString("base64")}`,
    },
    body: form,
  });

  if (!response.ok) {
    const body = await response.text();
    throw new Error(`Mailgun send failed: ${response.status} ${body}`);
  }

  return response.json();
}

export async function sendAccessTokenEmail({ email, token }) {
  const html = emailShell({
    eyebrow: "Quota Report Hub",
    title: "Your access token",
    intro: "Here is your personal access token. Paste it back into the quota-reporter setup to connect this machine to the shared auth pool.",
    details: [{ label: "Account", value: escapeHtml(email) }],
    code: { label: "Personal access token", value: escapeHtml(token) },
    action: "Paste this token into the quota-reporter setup prompt (or the hub's access-token field) to finish setup.",
    footer: "Keep it secret — anyone with this token can upload and fetch auth pool entries as you. The hub keeps only your most recent token per email.",
  });
  return sendMailgunEmail({
    to: email,
    subject: "Quota Report Hub access token",
    text: [
      "Your Quota Report Hub personal access token:",
      token,
      "",
      "Paste this token back into the quota-reporter setup to connect this machine to the shared auth pool.",
      "Keep it secret. Anyone with this token can upload and fetch auth pool entries as you.",
    ].join("\n"),
    html,
  });
}

function emailShell({ eyebrow, title, intro, details = [], code, action, footer }) {
  const detailRows = details
    .map(
      ({ label, value }) => `
        <tr>
          <td style="padding:10px 0;color:#6b7280;font-size:13px;line-height:18px;vertical-align:top;">${label}</td>
          <td style="padding:10px 0;color:#111827;font-size:14px;line-height:20px;font-weight:600;text-align:right;vertical-align:top;">${value}</td>
        </tr>`
    )
    .join("");
  const detailsTable = details.length
    ? `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:6px 0 22px;border-top:1px solid #eef2f7;border-bottom:1px solid #eef2f7;">
                  ${detailRows}
                </table>`
    : "";
  const codeBlock = code
    ? `<div style="margin:6px 0 22px;padding:18px 20px;border-radius:14px;background:#0f172a;">
                  <div style="color:#94a3b8;font-size:11px;line-height:16px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;margin:0 0 10px;">${code.label}</div>
                  <div style="font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;font-size:14px;line-height:22px;color:#e2e8f0;word-break:break-all;">${code.value}</div>
                </div>`
    : "";
  const actionBlock = action
    ? `<div style="padding:16px 18px;border-radius:14px;background:#f9fafb;border:1px solid #e5e7eb;color:#374151;font-size:14px;line-height:22px;">
                  ${action}
                </div>`
    : "";
  return `<!doctype html>
<html>
  <body style="margin:0;padding:0;background:#f3f4f6;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;color:#111827;">
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#f3f4f6;padding:32px 16px;">
      <tr>
        <td align="center">
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:560px;background:#ffffff;border:1px solid #e5e7eb;border-radius:20px;overflow:hidden;box-shadow:0 18px 50px rgba(15,23,42,0.08);">
            <tr>
              <td style="padding:28px 30px 22px;background:linear-gradient(135deg,#111827 0%,#334155 100%);">
                <div style="color:#cbd5e1;font-size:12px;line-height:16px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;">${eyebrow}</div>
                <h1 style="margin:10px 0 0;color:#ffffff;font-size:24px;line-height:30px;font-weight:750;">${title}</h1>
              </td>
            </tr>
            <tr>
              <td style="padding:28px 30px 30px;">
                <p style="margin:0 0 18px;color:#374151;font-size:15px;line-height:24px;">${intro}</p>
                ${detailsTable}
                ${codeBlock}
                ${actionBlock}
                <p style="margin:22px 0 0;color:#9ca3af;font-size:12px;line-height:18px;">${footer}</p>
              </td>
            </tr>
          </table>
        </td>
      </tr>
    </table>
  </body>
</html>`;
}

function escapeHtml(value) {
  return String(value)
    .replaceAll("&", "&amp;")
    .replaceAll("<", "&lt;")
    .replaceAll(">", "&gt;")
    .replaceAll('"', "&quot;")
    .replaceAll("'", "&#39;");
}

export async function sendAuthInvalidatedEmail({ email, entry, invalidatedSince }) {
  const source = entry?.source || "auth";
  const accountEmail = entry?.email || entry?.account_id || "unknown account";
  const plan = entry?.plan_name ? ` (${entry.plan_name})` : "";
  const safeSource = escapeHtml(source);
  const safeAccount = escapeHtml(accountEmail);
  const safePlan = escapeHtml(entry?.plan_name || "unknown");
  const safeInvalidatedSince = escapeHtml(invalidatedSince);
  const subject = `Quota Report Hub: ${source} auth needs login`;
  const text = [
    `Your ${source} auth in Quota Report Hub has been invalidated for more than 24 hours.`,
    "",
    `Account: ${accountEmail}${plan}`,
    `Invalidated since: ${invalidatedSince}`,
    "",
    "Please log in again on a machine with the quota-reporter skill installed so the hub can upload a refreshed auth.",
  ].join("\n");
  const html = emailShell({
    eyebrow: "Quota Report Hub",
    title: `${safeSource} login required`,
    intro: "This saved auth has been invalidated for more than 24 hours. It will stay out of rotation until the owner logs in again.",
    details: [
      { label: "Account", value: safeAccount },
      { label: "Plan", value: safePlan },
      { label: "Invalidated since", value: safeInvalidatedSince },
    ],
    action: "Please log in again on a machine with the quota-reporter skill installed. The next guard run will upload the refreshed auth automatically.",
    footer: "This reminder is sent at most once per 24 hours for the same account.",
  });
  return sendMailgunEmail({ to: email, subject, text, html });
}

// One digest to an owner: which pooled refresh tokens died in the last day, how long each had been
// unused, and what the provider said about it. Every interpolated value is escaped for the HTML body
// (account ids come from users' own credentials), and the plain-text part carries the same rows.
export async function sendDeathDigestEmail({ to, digest }) {
  const rows = digest.explained.map((death) => {
    const idle = Number.isFinite(death.idle_hours) ? `idle ${death.idle_hours.toFixed(1)} h` : "idle unknown";
    const plan = death.plan_name ? `${death.plan_name} · ` : "";
    return {
      account: death.account_id,
      when: death.observed_at,
      detail: `${plan}${idle} · ${death.refresh_error_code}`,
    };
  });
  const text = [
    `${rows.length} pooled refresh token${rows.length === 1 ? "" : "s"} died since ${digest.sinceIso}, with the provider's reason:`,
    "",
    ...rows.map((row) => `${row.when}  ${row.account}  ${row.detail}`),
    ...(digest.unexplained
      ? ["", `${digest.unexplained} more death${digest.unexplained === 1 ? "" : "s"} carried no refusal code (the hub never presented the token) and are not listed.`]
      : []),
    "",
    "refresh_token_expired: unused too long. refresh_token_reused: another holder spent that generation. refresh_token_invalidated: the session was ended, or a family revoked after a reuse.",
  ].join("\n");
  const html = emailShell({
    eyebrow: "Quota Report Hub",
    title: `${rows.length} refresh token${rows.length === 1 ? "" : "s"} died`,
    intro: "Each row is a pooled refresh token the hub presented and the provider refused, with how long it had been unused and the provider's own reason.",
    details: rows.map((row) => ({ label: escapeHtml(`${row.when} ${row.account}`), value: escapeHtml(row.detail) })),
    action: digest.unexplained
      ? `${digest.unexplained} further death${digest.unexplained === 1 ? "" : "s"} carried no refusal code and are not listed.`
      : "Nothing else died with a refusal code in this window.",
    footer: "Sent to owners at most once a day, and not at all when nothing died with a refusal code.",
  });
  return sendMailgunEmail({
    to,
    subject: `Quota Report Hub: ${rows.length} refresh token${rows.length === 1 ? "" : "s"} died`,
    text,
    html,
  });
}
