...
export async function tokenUsageHandlerImpl(req, res, deps = {
  ...
  try {
    ...
    // The version floor applies to usage exactly as it applies to quota. Usage is the one payload
    // an old client can get catastrophically wrong on its own -- differencing a cumulative counter
    // against state it lost charges a months-old session's whole history as fresh tokens -- and a
    // single such batch buries every real number on the dashboard. The refusal carries the upgrade
    // notice rather than an error: the client is not broken, it is old, and the notice plus the
    // reason are what make it update itself.
    //
    // activePhases is defaulted rather than required: every existing caller builds its own deps
    // object, and a handler that throws on a missing dependency would turn a version check into a
    // 503 on the ingest path -- the failure mode this gate exists to prevent.
    // Against THIS request's clock, not the wall clock. Every other time-dependent decision in this
    // handler already runs on deps.now(); leaving one of them reaching for new Date() meant the gate
    // was untestable by construction and only revealed it on the phase date, when three tests that
    // pin now to before the gate started returning 426.
    const phases = (deps.activePhases || activePhases)(receivedAt);
    if (clientNeedsUpgrade(normalized.client_version) && phases.reporter_gate) {
      // 426 and not 200: the client keeps the batch staged and retries it, so the usage measured
      // while it was stale is reported once it upgrades rather than thrown away. A 200 would be
      // acknowledged and a 400 discarded -- both lose the data over a problem that fixes itself
      // within one run.
      sendJson(res, 426, deps.withTokenUpgrade({
        ok: false,
        ignored: true,
        ...
        notices: withRepeatIntervals([upgradeNotice()]),
      }, authContext));
      return;
    }
...
export async function tokenUsageQueryHandlerImpl(req, res, deps = {
  ...
  try {
    ...
    sendJson(res, 200, deps.withTokenUpgrade({
      ...
      trend_granularity: result.trend_granularity,
      trend: result.trend,
