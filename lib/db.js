...
import { WEIGHTED_TOKENS_SQL } from "./premium-ratio.js";
import { STANDARD_MODEL_IDS } from "./model-tiers.js";
import {
  ...
  await client.batch([{
    ...
    sql: `
      INSERT INTO auth_pool_user_fetch_stats (
        requester_email, fetch_count, last_fetched_at, last_served_at
      ) VALUES (?, 1, ?, ?)
    ...
    `,
    args: [normalizedRequesterEmail, fetchedAt, SERVE_REASONS.has(String(reason)) ? fetchedAt : null],
...
export async function fetchPolicyInputs({ email, since }) {
  ...
  const standardPlaceholders = STANDARD_MODEL_IDS.map(() => "?").join(", ");
  const result = await client.execute({
    sql: `
      SELECT
        ...
        (
          SELECT COALESCE(SUM(${WEIGHTED_TOKENS_SQL}), 0)
          FROM token_usage_15m
        ...
        (
          ...
          FROM token_usage_15m
          WHERE hub_user_email = ?1 AND bucket_start >= ?2
            AND model_id NOT IN (${standardPlaceholders})
        ) AS premium_weighted
    `,
    args: [normalizedEmail, windowStart, ...STANDARD_MODEL_IDS],
  });
  const row = result.rows[0] || {};
  return {
    ...
    totalWeighted: Number(row.total_weighted || 0),
    premiumWeighted: Number(row.premium_weighted || 0),
  };
}
