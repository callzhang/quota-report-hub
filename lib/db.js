    ...
    schemaReady = (async () => {
      ...
      await client.execute(`
        CREATE TABLE quota_report_latest (
          ...
          one_week_reset_at TEXT,
          payload_json TEXT NOT NULL,
          PRIMARY KEY (source, hostname, auth_path)
        )
      `);
      await client.execute(`
        INSERT INTO quota_report_latest (
          source, hostname, auth_path, account_id, reporter_name, reported_at, email, name, plan_name,
        ...
        FROM (
          SELECT
            ...
            one_week_used_percent, one_week_remaining_percent, one_week_reset_at, payload_json,
            ROW_NUMBER() OVER (
              PARTITION BY source, hostname, auth_path
              ORDER BY reported_at DESC, event_id DESC
            ) AS row_number
          FROM quota_report_events
          WHERE auth_path IS NOT NULL
        )
        WHERE row_number = 1
      `);
    })();
...
export async function insertReport(rawReport) {
  ...
  await client.execute({
    sql: `
      ...
      ON CONFLICT(source, hostname, auth_path) DO UPDATE SET
