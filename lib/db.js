...
import { decryptAuthJson, deriveAuthPoolEntry, encryptAuthJson, normalizeTimestamp, pickBestAuthPoolCandidate, shouldReplaceAuthPoolEntry } from "./auth-pool.js";
import { authBlobKey, authBlobStorageConfigured, writeAuthBlob } from "./auth-blob-storage.js";
import { adminEmails, normalizeEmail, signTokenPayload, tokenHash, verifyTokenPayload } from "./company-auth.js";
  ...
  await client.batch([
    ...
    `
      INSERT INTO auth_pool_entries (
        ...
        uploader_email, reporter_name, hostname, checked_out_by, uploaded_at, encrypted_auth_json, iv, auth_tag, auth_blob_key, refresh_handoff_state
      )
      SELECT
        source, account_id, COALESCE(session_id, ''), email, name, plan_name, auth_last_refresh, auth_expires_at, has_refresh_token, digest,
        uploader_email, reporter_name, hostname, checked_out_by, uploaded_at, encrypted_auth_json, iv, auth_tag, auth_blob_key, NULL
...
// The invalidation record is the REFRESH token's death certificate: it drives the owner notification
// (re-login is the only cure) and, together with the access token's own expiry, the archive clock.
// It follows the refresh verdict alone -- an account whose RT is dead but whose access token still
// works keeps its record open while its row stays usable.
function refreshTokenRejected(report) {
  return refreshValidityFromReport(report) === "rejected";
}
...
export async function upsertAuthPoolQuota(report) {
  ...
  // Transitions are recorded HERE, not in the worker, because this is the one place all three
  // ingest paths meet -- api/auth/upload.js, lib/quota-ingest.js (client reports) and the worker.
  // Recording it in the worker missed every client-observed transition, and revivals are almost
  // always client-observed: an owner re-onboards and their own machine reports ok. That lost
  // time-to-repair, which is the measurement the table exists for. Measured 2026-09-08: bd@ and
  // derek@ both revived from a client at 02:00Z/02:04Z and left no row.
  //
  // Decided from the MERGED verdict for the same reason the invalidation clock below is: a client's
  // healthy probe does not lift a sticky central-refresh rejection, and the event log must agree
  // with the state it describes.
  await recordAuthPoolDeathTransition(previous, merged);
  ...
  const invalidationStatement = refreshTokenRejected(merged)
    ? {
        sql: `
...
export async function ensureSchema() {
  if (!schemaReady) {
    // A rejected promise cached here would poison the whole instance: every later call awaits the
    // same failure, so one transient error during startup takes a serverless container out for its
    // entire life while its neighbours serve fine. Clearing the handle on failure makes the next
    // caller retry instead, which matters most for the migrations below -- concurrently warm
    // instances can race each other through a table rebuild, and the loser must be able to recover.
...
async function detailTableIsKeyedByInstallation() {
  const existing = await client.execute(
    `SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'token_usage_15m'`,
  );
  return String(existing.rows[0]?.sql || "").includes("installation_id");
}
...
async function migrateSchema() {
  {
      ...
      await client.execute(`ALTER TABLE auth_pool_entries ADD COLUMN refresh_handoff_state TEXT`)
        .catch(() => {});
      await client.execute(`
      ...
      await client.execute(`
        CREATE TABLE IF NOT EXISTS token_usage_15m (
          ...
          PRIMARY KEY (hub_user_email, installation_id, provider, model_account_id, model_id, bucket_start)
        )
      `);
      // installation_id is in the KEY, not merely stored, and that is the whole point: without it
      // every machine reporting under one hub user writes into the same row, so a machine can add
      // to a bucket but can never correct it. That is what made the contaminated history
      // unrepairable from the machines that hold the only copy of the truth -- one machine's number
      // is a floor for a shared bucket, never the whole of it. With the grain at the machine, a
      // reporter can replace its own contribution and leave everyone else's alone.
      //
      // SQLite cannot change a primary key in place, so a database created before this rebuilds
      // once. Pre-existing rows carry installation_id = '' -- reported when machines were not
      // distinguishable, and therefore not attributable to any of them. A repair pass clears those
      // for its user's window along with its own rows, because the blob cannot be split and leaving
      // it would double-count against the attributed rows replacing it.
      if (!(await detailTableIsKeyedByInstallation())) {
        // Written to survive losing the race. Serverless instances warm independently, so two can
        // arrive here together: one drops the table the other is still copying from. Every step
        // tolerates having already been taken, and the whole thing is re-checked at the end -- if
        // somebody else finished it, that is success, not an error to poison the instance with.
        await client.execute(`
          CREATE TABLE IF NOT EXISTS token_usage_15m_rebuilt (
            hub_user_email TEXT NOT NULL,
            installation_id TEXT NOT NULL,
            provider TEXT NOT NULL,
            model_account_id TEXT NOT NULL,
        ...
        try {
          await client.execute(`
            INSERT OR IGNORE INTO token_usage_15m_rebuilt
            SELECT hub_user_email, '', provider, model_account_id, model_id, bucket_start,
                   input_tokens, output_tokens, cache_read_tokens, cache_write_tokens,
                   reasoning_tokens, total_tokens, updated_at
            FROM token_usage_15m
          `);
          await client.execute(`DROP TABLE IF EXISTS token_usage_15m`);
          await client.execute(`ALTER TABLE token_usage_15m_rebuilt RENAME TO token_usage_15m`);
        } catch (error) {
          // Losing the race looks like a failure from here -- the table we were copying from is
          // gone, or the name we were renaming to is taken -- but the work is done and the data is
          // in place. Only a table that is still not keyed by installation is a real failure.
          if (!(await detailTableIsKeyedByInstallation())) {
            throw error;
          }
        }
      ...
      `);
      // One row per Claude message, keyed on a SHA-256 of the provider's message id -- globally,
      // because a message happened once however many machines hold a copy of it. The Claude desktop
      // app mirrors a remote session into the laptop's transcripts byte for byte, so the laptop and
      // the remote host both report it; only a key on the message itself lets the hub count it
      // once. The row stays with whoever reported it first (installation, account, bucket), and the
      // token_usage_15m Claude rows are sums over this table rather than increments of their own.
      await client.execute(`
        CREATE TABLE IF NOT EXISTS token_usage_claude_messages (
      ...
      `);
      await client.execute(`
        CREATE INDEX IF NOT EXISTS token_usage_claude_messages_owner_idx
          ON token_usage_claude_messages (hub_user_email, installation_id, model_account_id, model_id, bucket_start)
      `);
      await client.execute(`
        CREATE INDEX IF NOT EXISTS token_usage_claude_messages_time_idx
          ON token_usage_claude_messages (bucket_start)
      `);
      await client.execute(`
      ...
      `);
      // A cooldown begins on the first refused request, not on the previous successful serve. It is
      // separate state so retries can preserve the original start and a completed serve can clear
      // the cycle without erasing audit history.
      await client.execute(`ALTER TABLE auth_pool_user_fetch_stats ADD COLUMN cooldown_started_at TEXT`)
        .catch(() => {});
      ...
      await client.execute(`ALTER TABLE auth_pool_user_fetch_stats ADD COLUMN client_version TEXT`)
        .catch(() => {});
      // Separate from the cooldown cycle. The reporting-debt clock must
      ...
      `);
      // Deaths as events, not as state. The table above holds only the CURRENT invalidation, so an
      // account someone re-onboards disappears from it entirely — which is how a cross-section of the
      // pool came to read "Pro accounts do not die" when the June-era snapshot had Pro dying at 75%
      // against Team's 77% (AUTH_TOKENS §3.7). This one never forgets, and it carries the join the hub
      // could not previously make: whether the hub had itself rotated the grant before the credential
      // died. Never pruned — the longitudinal view is the entire point.
      await client.execute(`
        CREATE TABLE IF NOT EXISTS auth_pool_death_events (
          id INTEGER PRIMARY KEY AUTOINCREMENT,
          source TEXT NOT NULL,
          account_id TEXT NOT NULL,
          event TEXT NOT NULL,
          ...
          plan_name TEXT,
          error TEXT,
          hub_last_refresh_at TEXT,
          last_healthy_probe_at TEXT,
          central_refresh_verdict TEXT
        )
      `);
      await client.execute(`
        CREATE INDEX IF NOT EXISTS auth_pool_death_events_account
        ON auth_pool_death_events (source, account_id, id)
      `);
      // Why the provider refused the refresh token, in its own words (refresh_token_expired /
      // _reused / _invalidated), so a death can be told apart as "unused too long" versus "someone
      // else spent it" versus "the session was ended". Added after the table shipped, hence ALTER.
      await client.execute(`ALTER TABLE auth_pool_death_events ADD COLUMN refresh_error_code TEXT`)
        .catch(() => {});
      // One row per refresh that actually reached the provider, from every server-side route
      // (worker, fetch-best, upload). Append-only and never pruned, for the same reason as the death
      // log: the question it answers is longitudinal. rt_age_seconds is how long the RT had been
      // unused when it was presented -- codex only, null elsewhere (see refreshTokenAgeSeconds).
      await client.execute(`
        CREATE TABLE IF NOT EXISTS auth_pool_refresh_attempts (
          id INTEGER PRIMARY KEY AUTOINCREMENT,
          source TEXT NOT NULL,
          account_id TEXT NOT NULL,
          ...
          rt_age_seconds INTEGER
        )
      `);
      await client.execute(`
        CREATE INDEX IF NOT EXISTS auth_pool_refresh_attempts_account
        ON auth_pool_refresh_attempts (source, account_id, id)
      `);
      await client.execute(`
      ...
      `);
      await client.execute(`ALTER TABLE pool_health_snapshots ADD COLUMN skipped_count INTEGER NOT NULL DEFAULT 0`)
        .catch(() => {});
      await client.execute(`
      ...
      await client.execute(`
        CREATE TABLE IF NOT EXISTS reporter_probe_heartbeats (
          ...
          client_version TEXT,
          client_sha TEXT,
          self_update_ok INTEGER,
          self_update_checked_at TEXT,
          self_update_error TEXT,
          consecutive_failures INTEGER NOT NULL DEFAULT 0,
          last_ok_at TEXT,
          PRIMARY KEY (source, reporter_key)
        )
      `);
      // ... and databases created before the column existed get it here, AFTER the table is known to
      // exist. client_version is a hand-maintained string nobody bumps reliably; the self-updater's
      // applied SHA is the fact.
      await client.execute(`ALTER TABLE reporter_probe_heartbeats ADD COLUMN client_sha TEXT`)
        .catch(() => {});
      // Whether the self-updater that keeps client_version and client_sha current is working. A
      // machine stuck on an old version and a machine nobody has bumped look identical without
      // these, and the reason it is stuck otherwise stays in a JSON file on that laptop.
      await client.execute(`ALTER TABLE reporter_probe_heartbeats ADD COLUMN self_update_ok INTEGER`)
        .catch(() => {});
      await client.execute(`ALTER TABLE reporter_probe_heartbeats ADD COLUMN self_update_checked_at TEXT`)
        .catch(() => {});
      await client.execute(`ALTER TABLE reporter_probe_heartbeats ADD COLUMN self_update_error TEXT`)
        .catch(() => {});
      // Which codex quota bucket this reporter last read. Readings from any bucket but the plan's
      // are refused before the event log, so this column is the only place a client-side observation
      // of one survives -- and the only way a newly introduced metered id becomes visible.
      await client.execute(`ALTER TABLE reporter_probe_heartbeats ADD COLUMN meter_limit_id TEXT`)
        .catch(() => {});
      await client.execute(`
      ...
      `);
      // A provider consumes an RT on refresh. This table serializes every hub refresh path per
      // credential generation, without storing the RT itself: the HMAC fingerprint is only useful
      // while a short lease is active and cannot be used to reconstruct the credential.
      await client.execute(`
        CREATE TABLE IF NOT EXISTS auth_pool_refresh_leases (
          source TEXT NOT NULL,
          account_id TEXT NOT NULL,
...
function tokenUsagePayloadDigest({ installationId, batchId, rows, messages }) {
  ...
  const canonicalMessages = messages
    .map((message) => ({
      ...
      model_account_id: message.model_account_id,
    ...
    .sort((left, right) => left.message_key.localeCompare(right.message_key));
  return crypto.createHash("sha256").update(JSON.stringify({
    installation_id: installationId,
    batch_id: batchId,
    rows: canonicalRows,
    // Only present when there are messages, so a rows-only batch keeps the digest it always had and
    // a retry that straddles a deploy is still recognised as the same batch.
    ...(canonicalMessages.length ? { messages: canonicalMessages } : {}),
  })).digest("hex");
}
...
export async function ingestTokenUsageBatch({
  ...
  const normalizedMessages = Array.from(messages || []);
  ...
  // A repair batch opens by clearing what it is about to replace, in the same transaction and
  // behind the same receipt guard as the rows themselves -- so a batch that is retried does not
  // clear twice, and a batch that never applies does not clear at all.
  //
  // Two deletes, because there are two kinds of row to displace. This machine's own rows are its
  // to correct. The installation_id = '' rows are what everyone reported before the grain was per
  // machine: an undifferentiated blob that cannot be split, cannot be attributed, and would
  // double-count against the attributed rows arriving now. Clearing them costs the contribution of
  // any machine that never repairs, and that is the price of a number that can be trusted -- the
  // blob is exactly the data we know to be wrong.
  if (replaceFrom) {
    for (const owner of [normalizedInstallationId, ""]) {
      statements.push({
        sql: `
          DELETE FROM token_usage_15m
          WHERE hub_user_email = ? AND installation_id = ? AND bucket_start >= ?
            AND EXISTS (
        ...
        `,
        args: [normalizedEmail, owner, String(replaceFrom), ...receiptGuardArgs],
      });
    }
    // And the Claude messages this machine was first to report in the window: it is about to send
    // them again, and a message still on file would be taken as already counted.
    statements.push({
      sql: `
        DELETE FROM token_usage_claude_messages
        WHERE hub_user_email = ? AND installation_id = ? AND bucket_start >= ?
          AND EXISTS (
            SELECT 1 FROM token_usage_batch_receipts
            WHERE hub_user_email = ? AND installation_id = ? AND batch_id = ?
      ...
      `,
      args: [normalizedEmail, normalizedInstallationId, String(replaceFrom), ...receiptGuardArgs],
    });
  }
  ...
  for (const row of normalizedRows) {
    statements.push({
      sql: `
        ...
        ON CONFLICT(hub_user_email, installation_id, provider, model_account_id, model_id, bucket_start)
        DO UPDATE SET
  ...
  // A message already on file keeps its owner and takes the larger of each counter: a mirror can
  // lag its original mid-stream, so whichever copy is fuller wins, and only once.
  for (const message of normalizedMessages) {
    statements.push({
      sql: `
        INSERT INTO token_usage_claude_messages (
          message_key, hub_user_email, installation_id, model_account_id, model_id, bucket_start,
        ...
        ON CONFLICT(message_key) DO UPDATE SET
          input_tokens = MAX(token_usage_claude_messages.input_tokens, excluded.input_tokens),
          output_tokens = MAX(token_usage_claude_messages.output_tokens, excluded.output_tokens),
          cache_read_tokens = MAX(token_usage_claude_messages.cache_read_tokens, excluded.cache_read_tokens),
          cache_write_tokens = MAX(token_usage_claude_messages.cache_write_tokens, excluded.cache_write_tokens),
          total_tokens = MAX(token_usage_claude_messages.input_tokens, excluded.input_tokens)
            + MAX(token_usage_claude_messages.output_tokens, excluded.output_tokens),
          updated_at = excluded.updated_at
      `,
      args: [
        message.message_key, normalizedEmail, normalizedInstallationId, message.model_account_id,
        message.model_id, message.bucket_start, message.input_tokens, message.output_tokens,
        message.cache_read_tokens, message.cache_write_tokens, message.total_tokens, receivedAt,
        ...receiptGuardArgs,
      ],
    });
  }
  // Then every Claude row those messages belong to is recomputed from the message table -- the rows
  // of whoever owns each message, which is not necessarily the machine sending this batch. Replace,
  // not add: the message table is the only source for these rows.
  if (normalizedMessages.length) {
    const keys = normalizedMessages.map((message) => message.message_key);
    statements.push({
      sql: `
        INSERT INTO token_usage_15m (
          hub_user_email, installation_id, provider, model_account_id, model_id, bucket_start,
        ...
        SELECT m.hub_user_email, m.installation_id, 'claude', m.model_account_id, m.model_id, m.bucket_start,
               SUM(m.input_tokens), SUM(m.output_tokens), SUM(m.cache_read_tokens), SUM(m.cache_write_tokens),
               0, SUM(m.total_tokens), ?
        FROM token_usage_claude_messages m
        JOIN (
          SELECT DISTINCT hub_user_email, installation_id, model_account_id, model_id, bucket_start
          FROM token_usage_claude_messages
          WHERE message_key IN (${keys.map(() => "?").join(", ")})
        ) touched USING (hub_user_email, installation_id, model_account_id, model_id, bucket_start)
        WHERE EXISTS (
          SELECT 1 FROM token_usage_batch_receipts
          WHERE hub_user_email = ? AND installation_id = ? AND batch_id = ?
            AND payload_digest = ? AND applied_at IS NULL
        )
        GROUP BY m.hub_user_email, m.installation_id, m.model_account_id, m.model_id, m.bucket_start
        ON CONFLICT(hub_user_email, installation_id, provider, model_account_id, model_id, bucket_start)
        DO UPDATE SET
          ...
          updated_at = excluded.updated_at
      `,
      args: [receivedAt, ...keys, ...receiptGuardArgs],
    });
  }
...
function numericTokenUsageRow(row, extraKeys = []) {
  ...
  // Money, not a counter: a fraction of a dollar, and the only figure on the page that is
  // comparable across providers and models. It is priced in SQL by the same MODEL_COST_SQL the
  // fetch gate spends -- see queryTokenUsage -- so what a reader sees is what the gate acted on.
  normalized.cost_usd = Number(row.cost_usd || 0);
  return normalized;
}

// How wide a range each bucket can cover before its own row count becomes the problem. A quarter
// hour over two days is 192 buckets; an hour over two weeks is 336; a day over the full 90-day
// detail window is 90. Multiply by the number of series on the chart and every case stays inside
// the trend budget for any group count anyone actually runs.
const TREND_GRANULARITY_LIMITS = [
  ["15m", 2 * 24 * 60 * 60 * 1000],
  ["hour", 21 * 24 * 60 * 60 * 1000],
];

export function trendGranularityFor({ start, end, granularity }) {
  const span = Date.parse(end) - Date.parse(start);
  if (!Number.isFinite(span)) return granularity;
  const cap = TREND_GRANULARITY_LIMITS.find(([unit]) => unit === granularity)?.[1];
  // Only ever coarsens. Asking for days over an hour-wide range is a strange request but an
  // answerable one, and quietly making it finer would be answering a different question.
  return cap === undefined || span <= cap
    ? granularity
    : (granularity === "15m" && span <= TREND_GRANULARITY_LIMITS[1][1] ? "hour" : "day");
}
...
export async function queryTokenUsage({
  ...
  // The trend is capped at TOKEN_USAGE_TREND_LIMIT rows, and a row is one bucket per series -- so
  // a range wide enough at a fine bucket used to be refused outright with "narrow the time range".
  // Refusing to draw is the worst answer available: the question ("what did the last month look
  // like?") is a perfectly good one, and it is the BUCKET that has to give, not the range. So the
  // trend coarsens itself to fit and the response says which bucket it actually used. Totals and
  // the breakdown are unaffected -- they never had a per-bucket row to blow the budget with.
  const trendGranularity = trendGranularityFor({ start, end, granularity });
  ...
  const counterSums = TOKEN_USAGE_COUNTERS
    ...
    .join(",\n");
  // The same expression the demand-share gate sums (fetchPolicyInputs). Tokens alone cannot answer
  // "what did this cost the pool": an hour of cache replay on Luna and an hour of Opus output are
  // three orders of magnitude apart, and a model nobody pays for -- an own-key DeepSeek, a
  // self-hosted Qwen -- prices at zero here exactly as it does in the gate. Sharing one expression
  // is what stops the page and the gate from reporting two different shares of the same week.
  const costSum = `COALESCE(SUM(${MODEL_COST_SQL}), 0) AS cost_usd`;

  const results = await client.batch([{
    sql: `
      WITH usage AS (${usageSource})
      SELECT ${counterSums}, ${costSum}
      FROM usage
    `,
    args: usageArgs,
  }, {
  ...
  return {
    ...
    // Named so the page can say what it is drawing. A chart at a bucket the reader did not choose
    // and is not told about is a chart that quietly lies about its own resolution.
...
export async function compactTokenUsage({
  ...
  for (const day of days) {
    ...
    const results = await client.batch([{
      ...
      // Past the detail window a message can no longer be re-reported (the ingest window is the
      // same 90 days), so its record has nothing left to deduplicate against.
      sql: `
        DELETE FROM token_usage_claude_messages
        WHERE bucket_start >= ? AND bucket_start < ? AND bucket_start < ?
      `,
...
// Admins are configuration, not deployment: they live in auth_admins so the dashboard can show
// the list and the owner can change it without a redeploy. Exactly one row carries role 'owner' —
// the person who set the service up — and ownership never moves through this API.
//
// Seeding runs once, on the first read of an empty table. ADMIN_EMAIL (the pre-database
// mechanism) wins so a live deployment keeps its admins, with the first listed becoming owner;
// without it, the earliest-created auth user — the first person to set the service up — becomes
// owner. A brand-new database with no users yet simply stays empty until one exists.
async function seedAdminsIfEmpty() {
  const existing = await client.execute(`SELECT COUNT(*) AS n FROM auth_admins`);
  if (Number(existing.rows[0]?.n || 0) > 0) return;
  let seeds = adminEmails().map((email, index) => ({ email, role: index === 0 ? "owner" : "admin" }));
  if (!seeds.length) {
    const first = await client.execute(`SELECT email FROM auth_users ORDER BY created_at ASC, email ASC LIMIT 1`);
    if (!first.rows[0]) return;
    seeds = [{ email: normalizeEmail(first.rows[0].email), role: "owner" }];
  }
  const seededAt = new Date().toISOString();
  await client.batch(seeds.map((seed) => ({
    sql: `INSERT INTO auth_admins (email, role, added_at, added_by) VALUES (?, ?, ?, 'seed') ON CONFLICT(email) DO NOTHING`,
    args: [seed.email, seed.role, seededAt],
  })));
}
...
export async function listAdmins() {
  ...
  const result = await client.execute(`
    SELECT email, role, added_at, added_by FROM auth_admins
    ORDER BY CASE role WHEN 'owner' THEN 0 ELSE 1 END, added_at ASC, email ASC
  `);
...
export async function adminRole(email) {
  await ensureSchema();
  await seedAdminsIfEmpty();
  const result = await client.execute({
    sql: `SELECT role FROM auth_admins WHERE email = ?`,
...
export async function addAdmin({ email, addedBy = null }) {
  ...
  await client.batch([{
    sql: `INSERT INTO auth_admins (email, role, added_at, added_by) VALUES (?, 'admin', ?, ?) ON CONFLICT(email) DO NOTHING`,
    args: [normalizeEmail(email), addedAt, addedBy],
  }, dashboardRevisionUpdate(addedAt, " AND changes() > 0")]);
}

export async function removeAdmin(email) {
  await ensureSchema();
  // role guard in SQL: the owner row is not removable, whatever the caller believed it was.
  await client.batch([{
    sql: `DELETE FROM auth_admins WHERE email = ? AND role != 'owner'`,
    args: [normalizeEmail(email)],
  ...
  await client.batch([{
    sql: `
      INSERT INTO pool_health_snapshots (
        captured_at, source, total, ok_count, hard_dead_count, other_err_count, skipped_count,
    ...
    args: [
      ...
      Number(snapshot.skipped_count || 0),
...
export async function poolHealthSnapshots({ limit = 96 } = {}) {
  ...
  const result = await client.execute({
    sql: `
      SELECT captured_at, source, total, ok_count, hard_dead_count, other_err_count, skipped_count,
             central_refresh_attempted, central_refresh_ok, central_refresh_rejected
  ...
  await client.batch([revisionGuard, {
    sql: `
      INSERT INTO reporter_probe_heartbeats (
        ...
        status, error, account_id, client_version, client_sha,
        self_update_ok, self_update_checked_at, self_update_error, meter_limit_id,
        consecutive_failures, last_ok_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      ON CONFLICT(source, reporter_key) DO UPDATE SET
        hostname = excluded.hostname,
        reporter_name = excluded.reporter_name,
        ...
        self_update_ok = excluded.self_update_ok,
        self_update_checked_at = excluded.self_update_checked_at,
        self_update_error = excluded.self_update_error,
        meter_limit_id = excluded.meter_limit_id,
    ...
    args: [
      ...
      heartbeat.client_sha || null,
      heartbeat.self_update_ok === null || heartbeat.self_update_ok === undefined
        ? null
        : (heartbeat.self_update_ok ? 1 : 0),
      heartbeat.self_update_checked_at || null,
      heartbeat.self_update_error || null,
      heartbeat.meter_limit_id || null,
      failed ? 1 : 0,
...
export async function reporterProbeHeartbeats({ limit = 200 } = {}) {
  ...
  const result = await client.execute({
    sql: `
      SELECT source, reporter_key, hostname, reporter_name, hub_user_email, last_run_at,
             status, error, account_id, client_version, client_sha,
             self_update_ok, self_update_checked_at, self_update_error,
             consecutive_failures, last_ok_at
  ...
  return result.rows.map((row) => ({
    ...
    client_sha: row.client_sha || null,
    self_update_ok: row.self_update_ok === null || row.self_update_ok === undefined
      ? null
      : Boolean(Number(row.self_update_ok)),
    self_update_checked_at: row.self_update_checked_at || null,
    self_update_error: row.self_update_error || null,
...
// The access token the pool holds for an account now: the one most recently recorded for it. Every
// write of the pooled blob -- upload, hub refresh, AT-only merge -- records its token, so the newest
// record is the current generation and every older one has been superseded.
export async function authPoolCurrentTokenFingerprint(source, accountId) {
  await ensureSchema();
  const result = await client.execute({
    sql: `
      SELECT fingerprint FROM auth_pool_token_fingerprints
      WHERE source = ? AND account_id = ?
  ...
  return result.rows[0] ? String(result.rows[0].fingerprint) : null;
}

// Claim a short, account-wide refresh lease. SQLite's conditional UPSERT is the lock: only an
// absent or expired row can become ours, so upload, fetch-best and the worker cannot concurrently
// present the same provider RT. Callers must release after their attempt; expiry is the crash-safe
// escape hatch for an interrupted serverless invocation.
export async function claimAuthPoolRefreshLease({
  source,
  accountId,
  ...
  const result = await client.execute({
    sql: `
      INSERT INTO auth_pool_refresh_leases (
        source, account_id, refresh_token_fingerprint, lease_id, claimed_at, expires_at
      ) VALUES (?, ?, ?, ?, ?, ?)
      ON CONFLICT(source, account_id) DO UPDATE SET
      ...
      WHERE auth_pool_refresh_leases.expires_at <= excluded.claimed_at
      RETURNING lease_id
    `,
    args: [String(source), String(accountId), String(refreshTokenFingerprint), String(leaseId), claimedAt, expiresAt],
  });
  return result.rows[0]?.lease_id === String(leaseId)
    ? { claimed: true, reason: null }
    : { claimed: false, reason: "refresh_in_progress" };
}

export async function releaseAuthPoolRefreshLease({ source, accountId, leaseId }) {
  await ensureSchema();
  await client.execute({
    sql: `DELETE FROM auth_pool_refresh_leases WHERE source = ? AND account_id = ? AND lease_id = ?`,
    args: [String(source), String(accountId), String(leaseId)],
  });
}

// Only the authenticated uploader may release this block. A client can set it before stripping a
// local RT, but cannot clear it until its activity monitor has observed a safe idle handoff.
export async function setAuthPoolRefreshHandoff({ source, accountId, uploaderEmail, state }) {
  await ensureSchema();
  const nextState = state === "pending" ? "pending" : null;
  const result = await client.execute({
    sql: `UPDATE auth_pool_entries
          SET refresh_handoff_state = ?
          WHERE source = ? AND account_id = ? AND uploader_email = ?`,
    args: [nextState, String(source), String(accountId), normalizeEmail(uploaderEmail)],
  });
  return { updated: Number(result.rowsAffected || 0) > 0, state: nextState };
}
...
async function mergeStrippedUploadIntoPooledEntry(rawEntry) {
  ...
  // The blob names an account; the token may belong to another. A borrower whose identity record has
  // drifted uploads the token it runs under the name it believes -- that is exactly how a live token
  // of one account was merged into a dead account's entry on 2026-09-05 and served under its name.
  // The hub issued the token and knows whose it is: a known fingerprint that maps elsewhere is refused.
  // An unknown token is accepted -- borrowers only ever hold pooled (known) tokens, so an unknown
  // access token is a fresh mint on the owner's own machine.
  const fingerprint = accessTokenFingerprint(rawEntry.auth_json, source);
  const owner = fingerprint ? await authPoolTokenOwner(source, fingerprint) : null;
  if (owner && owner.account_id !== incoming.account_id) {
    return { auth_json: null, reason: "stripped_access_token_belongs_to_another_account" };
  }
...
export async function upsertAuthPoolEntry(rawEntry) {
  await ensureSchema();
  // Whether the UPLOADER handed over a real refresh token. Read here, before the strip guard below
  // rewrites rawEntry with the pooled blob (which carries the stored RT and would always answer
  // "yes"), and not from derived.has_refresh_token (the placeholder is a non-empty string, so that
  // flag answers "yes" for an access-token-only upload). Only a genuine refresh token may override
  // the replacement rule for an incumbent the provider has already refused.
  const incomingHasRealRefreshToken = !isStrippedRefreshToken(rawEntry.auth_json, rawEntry.source);
  ...
  const existingResult = await client.execute({
    sql: `
      SELECT
        ...
        e.hostname,
        e.uploaded_at,
        e.refresh_handoff_state,
        -- Whether the worker has already presented THIS pooled refresh token and been refused.
        -- json_extract over payload_json for the same reason HEALTHY_POOL_ENTRY_SQL uses it: the
        -- sanitized report is the record, and no column carries the verdict for a live entry
        -- (auth_pool_death_events stores it only at a hard-dead transition).
        json_extract(q.payload_json, '$.usage_summary.central_refresh.auth_rejected')
          AS existing_refresh_rejected
      FROM auth_pool_entries e
      LEFT JOIN auth_pool_quota_latest q
        ON e.source = q.source AND e.account_id = q.account_id
      WHERE e.source = ? AND e.account_id = ? AND e.session_id = ?
    `,
  ...
  const existingRow = existingResult.rows[0]
    ? {
        ...
        refresh_handoff_state: existingResult.rows[0].refresh_handoff_state || null,
      }
    : null;
  // A pending handoff is still owned by the machine that had the RT in memory when it was
  // uploaded. Accepting a newer blob is useful (that machine may have re-minted), but handing the
  // completion authority to its latest uploader would let another machine's idle proof unfreeze
  // the original owner's RT generation.
  const effectiveUploaderEmail = existingRow?.refresh_handoff_state === "pending"
    ? existingRow.uploader_email
    : accountUploaderEmail;
  // SQLite renders a JSON true as 1; an absent report or an absent verdict reads null -> not rejected.
  const existingRefreshRejected = existingResult.rows[0]?.existing_refresh_rejected === 1
    || existingResult.rows[0]?.existing_refresh_rejected === true;
  if (!shouldReplaceAuthPoolEntry(existingRow, derived, { existingRefreshRejected, incomingHasRealRefreshToken })) {
    const uploaderChanged = effectiveUploaderEmail !== null && effectiveUploaderEmail !== existingRow.uploader_email;
    ...
    if (uploaderChanged || reporterChanged || hostnameChanged || refreshCapabilityChanged) {
      ...
      const nextUploader = effectiveUploaderEmail || existingRow.uploader_email;
    ...
    return {
      ...
      uploader_email: existingRow.uploader_email || effectiveUploaderEmail,
  ...
  await client.batch([...cleanupStatements, {
    sql: `
      ...
      ON CONFLICT(${authPoolPkColumns}) DO UPDATE SET
        ...
        refresh_handoff_state = COALESCE(excluded.refresh_handoff_state, auth_pool_entries.refresh_handoff_state)
    `,
    args: [
      derived.source,
      derived.account_id,
      ...
      authTag,
      blobKey,
      rawEntry.refresh_handoff_state === "pending" ? "pending" : null,
    ],
  ...
  return {
    ...
    refresh_handoff_state: rawEntry.refresh_handoff_state === "pending" ? "pending" : existingRow?.refresh_handoff_state || null,
    deduplicated: false,
  };
}
...
export async function authPoolEntries() {
  ...
  return result.rows.map((row) => ({
    ...
    refresh_handoff_state: row.refresh_handoff_state || null,
  }));
}
  ...
  await client.batch([{
    ...
    sql: `
      INSERT INTO auth_pool_user_fetch_stats (
        requester_email, fetch_count, last_fetched_at, client_version, last_new_account_at,
        cooldown_started_at
      ) VALUES (?1, 1, ?2, ?3, ?4, ?5)
      ON CONFLICT(requester_email) DO UPDATE SET
        ...
        cooldown_started_at = CASE
          WHEN ?6 = 1 THEN NULL
          WHEN excluded.cooldown_started_at IS NOT NULL
            THEN COALESCE(auth_pool_user_fetch_stats.cooldown_started_at, excluded.cooldown_started_at)
          ELSE auth_pool_user_fetch_stats.cooldown_started_at
        END
    ...
    args: [
      ...
      COOLDOWN_REASONS.has(String(reason)) ? fetchedAt : null,
      COOLDOWN_COMPLETION_REASONS.has(String(reason)) ? 1 : 0,
    ],
...
export async function authPoolActiveRequesterAssignments() {
  await ensureSchema();
  const result = await client.execute(`
    SELECT source, requester_email, requester_id, fetched_at, active_account_id, reason
    FROM auth_pool_requester_assignments
...
function centralRefreshVerdictOf(report) {
  const central = report?.usage_summary?.central_refresh;
  if (!central?.attempted) {
    return "not_attempted";
  }
  if (central.ok) {
    return "ok";
  }
  return central.auth_rejected ? "rejected" : "failed";
}

// One appended row per transition into or out of hard-dead. Why a death happened is deliberately NOT
// stored: the rule for splitting "the hub rotated this grant itself" from "something outside the hub
// did" is still open (AUTH_TOKENS §3.7), and a verdict computed by today's rule would silently outlive
// it. Facts here, judgement in the query.
export async function recordAuthPoolDeathEvent({
  source,
  accountId,
  event,
  observedAt,
  planName = null,
  error = null,
  hubLastRefreshAt = null,
  lastHealthyProbeAt = null,
  centralRefreshVerdict = null,
  refreshErrorCode = null,
}) {
  await ensureSchema();
  await client.execute({
    sql: `
      INSERT INTO auth_pool_death_events (
        source, account_id, event, observed_at, plan_name, error,
        hub_last_refresh_at, last_healthy_probe_at, central_refresh_verdict, refresh_error_code
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `,
    args: [
      String(source),
...
async function recordAuthPoolDeathTransition(previous, merged) {
  const wasHard = Boolean(previous) && isHardAuthError(previous.error);
  const isHard = merged?.status === "error" && isHardAuthError(merged.error);
  if (wasHard === isHard) {
    return;
  }
  // The plan is read from the credential, not from the report: a report's plan_name comes from the
  // probe payload, the entry's from the id_token claim, and analysis keyed on plan has to use the
  // credential one. Costs a read, but only on a transition -- ~1.5 a day against ~2,000 reports.
  let planName = merged?.plan_name ?? null;
  try {
    const entry = await client.execute({
      sql: `SELECT plan_name FROM auth_pool_entries WHERE source = ? AND account_id = ? LIMIT 1`,
      args: [String(merged.source), String(merged.account_id)],
    });
    if (entry.rows[0]?.plan_name) {
      planName = entry.rows[0].plan_name;
    }
  } catch {
    // A missing entry is normal for a credential already deleted from the pool; keep the report's.
  }
  await recordAuthPoolDeathEvent({
    source: merged.source,
    accountId: merged.account_id,
    event: isHard ? "death" : "revival",
    observedAt: merged.reported_at || new Date().toISOString(),
    planName,
    error: isHard ? merged.error ?? null : null,
    // `auth_last_refresh` is NOT the same fact on both sources, and the difference is semantic rather
    // than cosmetic. codex writes an ISO timestamp of the last rotation. claude writes a ms-epoch
    // string mirroring claudeAiOauth.expiresAt -- an EXPIRY, in the future. Normalising that to ISO
    // only makes a wrong value parseable: 52 of the 77 rows this column had accumulated were claude
    // epochs, and one of them converts to 2026-09-29 for a death observed on 2026-09-12, so every
    // rotation-to-death interval computed from a claude row would come out negative. Recorded only
    // where the field means what the column name says; claude gets null, which is the truth -- the
    // hub does not know when that grant was last rotated.
    //
    // Even for codex, read it as "the grant's last known rotation, by WHOEVER performed it": the
    // blob's `last_refresh` is written both by the hub's central refresh and by a client's captured
    // refresh. It is not "the hub refreshed at T", and a classifier must not treat it as such (§3.7).
    hubLastRefreshAt: merged.source === "codex" ? normalizeTimestamp(merged.auth_last_refresh) : null,
    lastHealthyProbeAt: previous?.status === "ok" ? previous.reported_at ?? null : null,
    // Only on a death. mergeLatestReport keeps a central-refresh rejection sticky, so a revival
    // inherits the verdict that belonged to the credential that just died -- the first row this
    // table ever recorded (hr@stardust.ai, 2026-09-08) came out "revival ... rejected", which reads
    // as a contradiction and is really stale evidence about a credential the owner has replaced.
    centralRefreshVerdict: isHard ? centralRefreshVerdictOf(merged) : null,
    // The provider's reason travels with the verdict and follows the same rule: only on a death.
    refreshErrorCode: isHard ? merged?.usage_summary?.central_refresh?.provider_error_code ?? null : null,
  });
}

export async function recordAuthPoolRefreshAttempt({ source, accountId, path = null, attemptedAt, rtAgeSeconds = null, result }) {
  await ensureSchema();
  await client.execute({
    sql: `
      INSERT INTO auth_pool_refresh_attempts (
        source, account_id, attempted_at, path, ok, http_status, provider_error_code, rt_age_seconds
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `,
    args: [
      String(source),
      ...
      result?.ok ? null : result?.provider_error_code ?? null,
      Number.isFinite(rtAgeSeconds) ? Math.round(rtAgeSeconds) : null,
    ],
  });
}

export async function authPoolRefreshAttempts({ source = null, accountId = null, since = null, limit = 500 } = {}) {
  await ensureSchema();
  const conditions = [];
  const args = [];
  if (source) {
  ...
  args.push(Math.max(1, Math.min(Number(limit) || 500, 5000)));
  const result = await client.execute({
    sql: `
      SELECT id, source, account_id, attempted_at, path, ok, http_status, provider_error_code, rt_age_seconds
      FROM auth_pool_refresh_attempts
  ...
  return result.rows.map((row) => ({
    ...
    rt_age_seconds: row.rt_age_seconds === null ? null : Number(row.rt_age_seconds),
  }));
}

export async function authPoolDeathEvents({ since = null, limit = 500 } = {}) {
  await ensureSchema();
  const conditions = [];
  const args = [];
  if (since) {
  ...
  const result = await client.execute({
    sql: `
      SELECT id, source, account_id, event, observed_at, plan_name, error,
             hub_last_refresh_at, last_healthy_probe_at, central_refresh_verdict, refresh_error_code
      FROM auth_pool_death_events
      ${conditions.length ? `WHERE ${conditions.join(" AND ")}` : ""}
      ORDER BY id DESC
      LIMIT ?
    `,
  ...
  return result.rows.map((row) => ({
    ...
    plan_name: row.plan_name,
    error: row.error,
    hub_last_refresh_at: row.hub_last_refresh_at,
    last_healthy_probe_at: row.last_healthy_probe_at,
    central_refresh_verdict: row.central_refresh_verdict,
    refresh_error_code: row.refresh_error_code ?? null,
  }));
}
...
// `spendSince` is the demand-share window's start, passed in rather than computed here so the page
// and the gate cannot end up measuring different weeks. The spend join is the same bucket_start
// range scan fetchPolicyInputs already runs on every fetch-best call, grouped by user instead of
// filtered to one -- the token_usage_15m_time_idx prefix carries both. Seven days sits inside the
// 90-day detail window, so the daily rollups are not in play and the aggregate cannot double-count.
export async function authUsersList({ spendSince }) {
  if (typeof spendSince !== "string" || !spendSince) {
    throw new TypeError("authUsersList requires the spend window start");
  }
  await ensureSchema();
  const result = await client.execute({
    sql: `
      ...
      LEFT JOIN (
        SELECT hub_user_email, SUM(${MODEL_COST_SQL}) AS spend_usd
        FROM token_usage_15m
        WHERE bucket_start >= ?
        GROUP BY hub_user_email
      ) c ON c.hub_user_email = u.email
...
// The first policy refusal starts one fixed wait. Repeated refusals preserve its original timestamp;
// handing over auth completes and clears that cycle so the next request must queue again.
const COOLDOWN_REASONS = new Set(["demand_share_cooldown", "contribution_cooldown"]);
const COOLDOWN_COMPLETION_REASONS = new Set(["served", "refreshed_current"]);
...
export async function fetchPolicyInputs({ email, since }) {
  ...
  const result = await client.execute({
    sql: `
      SELECT
        ...
        (SELECT cooldown_started_at FROM auth_pool_user_fetch_stats WHERE requester_email = ?1) AS cooldown_started_at,
  ...
  return {
    ...
    cooldownStartedAt: row.cooldown_started_at || null,
