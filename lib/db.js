...
async function migrateAuthPoolEntriesTableShape() {
  ...
  await client.batch([
    ...
    `
      ...
      SELECT
        source, account_id, COALESCE(session_id, ''), email, name, plan_name, auth_last_refresh, auth_expires_at, COALESCE(has_refresh_token, 0), digest,
        uploader_email, reporter_name, hostname, checked_out_by, uploaded_at, encrypted_auth_json, iv, auth_tag, auth_blob_key
...
export async function ensureSchema() {
  if (!schemaReady) {
    schemaReady = (async () => {
      ...
      await client.execute(`ALTER TABLE auth_pool_entries ADD COLUMN has_refresh_token INTEGER NOT NULL DEFAULT 0`)
        .catch(() => {});
...
export async function upsertAuthPoolEntry(rawEntry) {
  ...
  const existingRow = existingResult.rows[0]
    ? {
        ...
        has_refresh_token: Boolean(existingResult.rows[0].has_refresh_token),
