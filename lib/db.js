...
export async function setFeatureFlag(key, value, updatedBy = null) {
  ...
  await client.execute({
    ...
    `,
    args: [String(key), value ? "true" : "false", new Date().toISOString(), updatedBy],
  });
  return { key: String(key), value: Boolean(value) };
}
...
export async function authPoolEntrySummaries() {
  await ensureSchema();
  const result = await client.execute(`
    SELECT
      source,
...
export async function bestAuthPoolEntry(options = {}) {
  ...
  const pool = await authPoolEntrySummaries();
...
export async function issueApiToken(email) {
  ...
  // Do NOT evict existing tokens on issue — eviction happens on first USE (authenticateApiToken
  // revokes every older token for the email). This way a reissued-but-never-pasted token cannot lock
  // out the currently-working one: the old token keeps working until the new one is actually used.
  await client.execute({
    sql: `
...
// Activate-on-use eviction: using a token revokes every OLDER token for the same email. Paired with
// NOT evicting on issue (see issueApiToken), so a reissued-but-unused token never locks out the
// currently-working one — a newer token only supersedes the old once it is actually used.
async function revokeSupersededApiTokens(email, usedTokenCreatedAt) {
  if (!email || !usedTokenCreatedAt) {
    return;
  }
  await client.execute({
    sql: `DELETE FROM auth_api_tokens WHERE email = ? AND created_at < ?`,
    args: [email, usedTokenCreatedAt],
  });
}
...
export async function authenticateApiToken(rawToken) {
  ...
  if (verified) {
    ...
    await revokeSupersededApiTokens(row.email, row.created_at);
    return {
      email: row.email,
      created_at: row.created_at,
      last_used_at: usedAt,
