...
export async function ensureSchema() {
  if (!schemaReady) {
    schemaReady = (async () => {
      ...
      const existingDetail = await client.execute(
        `SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'token_usage_15m'`,
      );
      if (!String(existingDetail.rows[0]?.sql || "").includes("installation_id")) {
        await client.execute(`
          CREATE TABLE token_usage_15m_rebuilt (
            hub_user_email TEXT NOT NULL,
            installation_id TEXT NOT NULL,
            provider TEXT NOT NULL,
            model_account_id TEXT NOT NULL,
        ...
        `);
        await client.execute(`
          INSERT INTO token_usage_15m_rebuilt
        ...
        `);
        await client.execute(`DROP TABLE token_usage_15m`);
