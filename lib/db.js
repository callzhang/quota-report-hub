...
export async function upsertAuthPoolEntry(rawEntry) {
  ...
  if (!shouldReplaceAuthPoolEntry(existingRow, derived)) {
    if (existingRow.has_refresh_token === null) {
      const repairedAt = new Date().toISOString();
      await client.batch([
        dashboardRevisionUpdate(
          repairedAt,
          " AND EXISTS (SELECT 1 FROM auth_pool_entries WHERE source = ? AND account_id = ? AND session_id = ? AND has_refresh_token IS NULL)",
          [existingRow.source, existingRow.account_id, existingRow.session_id],
        ), {
        sql: `UPDATE auth_pool_entries SET has_refresh_token = ? WHERE source = ? AND account_id = ? AND session_id = ? AND has_refresh_token IS NULL`,
        args: [derived.has_refresh_token ? 1 : 0, existingRow.source, existingRow.account_id, existingRow.session_id],
      }]);
