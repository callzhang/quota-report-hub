...
export async function authPoolQuotaEvents({ source, accountId, since, limit = 96 } = {}) {
  await ensureSchema();
  const exactSource = String(source || "").trim();
  const exactAccountId = String(accountId || "").trim();
  const exactSince = String(since || "").trim();
  if (!exactSource || !exactAccountId || !exactSince) {
    throw new TypeError("source, accountId, and since are required");
  }
  ...
  const result = await client.execute({
    sql: `
      ...
      WHERE source = ? AND account_id = ? AND reported_at >= ?
      ORDER BY reported_at DESC
      LIMIT ?
    `,
    args: [exactSource, exactAccountId, exactSince, boundedLimit],
  });
  return result.rows.map((row) => rowToReport(row)).reverse();
}
