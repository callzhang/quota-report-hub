...
import { normalizeEmail, opaqueToken, signTokenPayload, tokenHash, verifyTokenPayload } from "./company-auth.js";
...
export async function hasUploadedAuth({ source, uploaderEmail }) {
  await ensureSchema();
  const result = await client.execute({
    sql: `SELECT 1 FROM auth_pool_entries WHERE source = ? AND uploader_email = ? LIMIT 1`,
    args: [String(source), normalizeEmail(uploaderEmail)],
  });
  return result.rows.length > 0;
}

export async function getInvalidatedUploaderEntry({ source, uploaderEmail }) {
  await ensureSchema();
  const result = await client.execute({
    sql: `
      SELECT
