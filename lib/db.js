...
async function upsertReporterAssignment(report) {
...
export async function upsertAuthPoolQuota(report) {
  ...
  await upsertReporterAssignment(incoming);
  ...
  if (isHardAuthInvalidationReport(incoming)) {
    await upsertInvalidatedAuthState({
      ...
      invalidatedAt: incoming.reported_at,
      error: incoming.error,
      bumpRevision: false,
    });
  } else {
    await clearInvalidatedAuthState({ source: incoming.source, accountId: incoming.account_id, bumpRevision: false });
  }
}
...
export async function setFeatureFlag(key, value, updatedBy = null) {
  ...
  }, dashboardRevisionUpdate(updatedAt)]);
  return { key: normalizedKey, value: Boolean(value) };
}
...
export async function recordAuthPoolFetch({
  ...
  await client.execute(dashboardRevisionUpdate(fetchedAt));
}
...
export async function upsertInvalidatedAuthState({ source, accountId, invalidatedAt, error = null, bumpRevision = true }) {
  await ensureSchema();
  const normalizedSource = String(source);
  const normalizedAccountId = String(accountId);
  ...
  if (bumpRevision && Number(write.rowsAffected || 0) > 0) {
    await client.execute(dashboardRevisionUpdate(firstInvalidatedAt));
  }
  const result = await client.execute({
    sql: `
...
export async function markInvalidatedAuthNotified({ source, accountId, notifiedAt }) {
  ...
  if (Number(write.rowsAffected || 0) > 0) {
    await client.execute(dashboardRevisionUpdate(normalizedNotifiedAt));
  }
}

export async function clearInvalidatedAuthState({ source, accountId, bumpRevision = true }) {
  await ensureSchema();
  const write = await client.execute({
    sql: `
  ...
  if (bumpRevision && Number(write.rowsAffected || 0) > 0) {
    await client.execute(dashboardRevisionUpdate());
  }
}
