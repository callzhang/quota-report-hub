...
export async function ensureSchema() {
  if (!schemaReady) {
    schemaReady = (async () => {
      ...
      for (const col of ['session_id', 'checked_out_by']) {
        await client.execute(`ALTER TABLE auth_pool_entries ADD COLUMN ${col} TEXT`)
          .catch(() => {}); // ignore if column already exists
      }
      // Migrate PK from (source, account_id) to (source, account_id, session_id) if needed
      const tableInfo = await client.execute(
        `SELECT sql FROM sqlite_master WHERE type='table' AND name='auth_pool_entries'`
      );
      const createSql = String(tableInfo.rows[0]?.sql || '');
      if (!createSql.includes('session_id')) {
        await client.execute(`
          CREATE TABLE auth_pool_entries_new (
            source TEXT NOT NULL,
            account_id TEXT NOT NULL,
            session_id TEXT NOT NULL DEFAULT '',
            email TEXT,
        ...
        `);
        await client.execute(`
          INSERT INTO auth_pool_entries_new
            (source, account_id, session_id, email, name, plan_name, auth_last_refresh, digest,
             uploader_email, reporter_name, hostname, checked_out_by, uploaded_at,
             encrypted_auth_json, iv, auth_tag)
          SELECT source, account_id, COALESCE(session_id, ''), email, name, plan_name, auth_last_refresh, digest,
                 uploader_email, reporter_name, hostname, checked_out_by, uploaded_at,
                 encrypted_auth_json, iv, auth_tag
          FROM auth_pool_entries
        `);
        await client.execute(`DROP TABLE auth_pool_entries`);
        await client.execute(`ALTER TABLE auth_pool_entries_new RENAME TO auth_pool_entries`);
      }
      await client.execute(`
  ...
  await client.execute({
    sql: `
      INSERT INTO auth_pool_entries (
        source, account_id, session_id, email, name, plan_name, auth_last_refresh, digest, uploader_email,
        reporter_name, hostname, uploaded_at, encrypted_auth_json, iv, auth_tag
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      ON CONFLICT(source, account_id, session_id) DO UPDATE SET
        email = excluded.email,
        name = excluded.name,
        plan_name = excluded.plan_name,
        auth_last_refresh = excluded.auth_last_refresh,
