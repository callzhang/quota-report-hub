  ...
  if (!schemaReady) {
    schemaReady = (async () => {
      ...
      await client.execute(`UPDATE auth_pool_entries SET session_id = '' WHERE session_id IS NULL`)
        .catch(() => {});
      // Detect actual PK columns (old DBs have PK on source,account_id)
      try {
...
export async function bestAuthPoolEntry(options = {}) {
  ...
  const recentFetchWindowSeconds = Number(options.recent_fetch_window_seconds ?? 5 * 60 * 60);
  const recentServedCounts = await authPoolRecentServedCounts({
    source: options.source || "codex",
    since: Number.isFinite(recentFetchWindowSeconds) && recentFetchWindowSeconds > 0
      ? new Date(Date.now() - recentFetchWindowSeconds * 1000).toISOString()
      : null,
  });
  ...
  const sql = dedupe
    ? `
      ...
      FROM (
        SELECT *,
          ROW_NUMBER() OVER (
            PARTITION BY source, requester_email
