...
export async function upsertAuthPoolQuota(report) {
  ...
  if (isHardAuthInvalidationReport(merged)) {
    await upsertInvalidatedAuthState({
      source: merged.source,
      accountId: merged.account_id,
      invalidatedAt: merged.reported_at,
      error: merged.error,
    });
  } else {
    await clearInvalidatedAuthState({ source: merged.source, accountId: merged.account_id });
  }
}
...
export async function deleteAuthPoolEntry({ source, accountId }) {
  await ensureSchema();
  const normalizedSource = String(source);
  const normalizedAccountId = String(accountId);
  const existing = await authPoolEntry(normalizedSource, normalizedAccountId);
  await client.batch([
    {
