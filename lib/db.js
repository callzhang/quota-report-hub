...
async function upsertAuthPoolQuota(report) {
...
export async function upsertAuthPoolEntry(rawEntry) {
  ...
  if (rawEntry.quota_payload) {
    const quota = sanitizeReport({
      ...rawEntry.quota_payload,
      ...
      hostname: derived.hostname || rawEntry.hostname || "unknown-host",
      reporter_name: derived.reporter_name || rawEntry.reporter_name || "unknown",
    });
    await upsertAuthPoolQuota(quota);
  }
