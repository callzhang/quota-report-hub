...
export async function ingestTokenUsageBatch({
  ...
  statements.push({
    sql: `
      INSERT INTO token_usage_reporter_state (hub_user_email, last_reported_at)
      SELECT ?, ?
      WHERE EXISTS (
        SELECT 1 FROM token_usage_batch_receipts
        WHERE hub_user_email = ? AND installation_id = ? AND batch_id = ?
      ...
      ON CONFLICT(hub_user_email) DO UPDATE SET
        ...
        END
    `,
    args: [normalizedEmail, receivedAt, ...receiptGuardArgs],
  });
  statements.push({
    sql: `
...
export async function queryTokenUsage({
  ...
  const results = await client.batch([{
    ...
    sql: `
      WITH usage AS (${usageSource}), per_user AS (
        SELECT hub_user_email, COALESCE(SUM(total_tokens), 0) AS total_tokens
        FROM usage
        GROUP BY hub_user_email
      )
      SELECT
        users.email AS hub_user_email,
        state.last_reported_at,
        COALESCE(per_user.total_tokens, 0) AS total_tokens
      FROM auth_users AS users
      LEFT JOIN token_usage_reporter_state AS state
        ON state.hub_user_email = users.email
      LEFT JOIN per_user
        ON per_user.hub_user_email = users.email
