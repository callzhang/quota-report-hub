import { PREMIUM_MODEL_IDS, STANDARD_MODEL_IDS, SUGGESTED_STANDARD_MODEL_IDS, isPremiumModel } from "./model-tiers.js";
...
// Per-notice repeat interval, chosen by the hub rather than the client: urgency is a policy
// judgement, and a client that picks its own cadence cannot be re-tuned without a release.
export const NOTICE_REPEAT_SECONDS = Object.freeze({
  usage_reporting_required: 60 * 60,
  reporter_upgrade_required: 6 * 60 * 60,
  premium_ratio_warning: 6 * 60 * 60,
  premium_ratio_cooldown: 6 * 60 * 60,
});
...
function withRepeatIntervals(notices) {
  return notices.map((notice) => ({
    ...notice,
    repeat_seconds: NOTICE_REPEAT_SECONDS[notice.code] ?? 6 * 60 * 60,
  }));
}
...
function ratioNotice({ share, cooldownActive }) {
  ...
  const fix = `把默认模型换成 ${SUGGESTED_EXAMPLES} 就能降下来。`;
  if (cooldownActive) {
    return {
      code: "premium_ratio_cooldown",
      title: "取号冷却中",
      message:
        `你近 ${PREMIUM_RATIO_WINDOW_DAYS} 天高级模型（${PREMIUM_EXAMPLES} 等）占比 ${percent}%，` +
        `超过 ${target}% 上限，现在每 ${PREMIUM_RATIO_COOLDOWN_MINUTES} 分钟才能取一次号。` +
        `${fix}降到 ${target}% 以下即恢复。`,
    };
  }
  return {
    ...
    title: "高级模型占比偏高",
    message:
      `你近 ${PREMIUM_RATIO_WINDOW_DAYS} 天高级模型（${PREMIUM_EXAMPLES} 等）占比 ${percent}%，目标 ≤ ${target}%。` +
      `${PHASE_RATIO_COOLDOWN_AT.slice(0, 10)} 起超标将限制为每 ${PREMIUM_RATIO_COOLDOWN_MINUTES} 分钟才能取一次号。${fix}`,
  };
}
