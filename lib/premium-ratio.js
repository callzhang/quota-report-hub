...
export function evaluateFetchPolicy({
  ...
  // Scarcity makes a user subject to a cooldown, but it does not mean this request is blocked.
  // The first request and the first request after the interval are served. Calling those requests
  // "已限速" immediately before handing back an auth makes the two notices contradict each other.
  const cooldownBlocked = enforcementEnabled
    && cooldownConfigured
    && servedAgeMs !== null
    && servedAgeMs < cooldownMs;
