...
// A share of total demand needs no threshold in dollars, and scales itself as the team and the pool
// change size. At K=2.5 with 13 active users the line sits at 19% of all demand: on current data
// that is the top two consumers and nobody else.
export const DEMAND_SHARE_TOLERANCE = 2.5;

// A ceiling on top of the per-head line, which otherwise exceeds 100% below three active users and
// would make the rule unreachable exactly when one person IS the shortage. Binds for three to five
// active users; beyond that the per-head line is stricter anyway.
export const DEMAND_SHARE_CEILING = 0.5;

// "Fair share" presupposes somebody to be fair to. With one or two people drawing on the pool,
// holding the bigger one back frees capacity for nobody -- the pool drains at the same rate and the
// only effect is that work stops sooner. A shortage that one person drives alone is a signal to add
// accounts, not to throttle the only user.
export const DEMAND_SHARE_MIN_ACTIVE_USERS = 3;
...
export function evaluateFetchPolicy({
  ...
  const fairShare = activeUsers >= DEMAND_SHARE_MIN_ACTIVE_USERS
    ? Math.min(DEMAND_SHARE_TOLERANCE / activeUsers, DEMAND_SHARE_CEILING)
    : Infinity;
