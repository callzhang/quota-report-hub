...
export function evaluateFetchPolicy({
  ...
  // The cooldown is a rationing rule, so it only bites while there is something to ration. The
  // warning still goes out during abundance -- that is what gives people time to change habits
  // before the pool tightens, instead of being throttled the first day it does.
  const cooldownActive = phases.ratio_cooldown && poolScarce;

  if (overThreshold) {
    notices.push(ratioNotice({ share, cooldownActive }));
    if (cooldownActive) {
