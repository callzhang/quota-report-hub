...
export function deriveReporterHealth(heartbeat, generatedAt = new Date().toISOString()) {
  ...
  const base = {
    ...
    // Carried beside the version because the two answer different questions. The version says what
    // this machine runs; these say whether it is capable of ever running anything else.
    self_update_ok: heartbeat?.self_update_ok === null || heartbeat?.self_update_ok === undefined
      ? null
      : Boolean(heartbeat.self_update_ok),
    self_update_checked_at: toIsoOrNull(heartbeat?.self_update_checked_at),
    self_update_error: heartbeat?.self_update_error || null,
