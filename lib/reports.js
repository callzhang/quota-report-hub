...
export function authPoolStatusPayload(entries, reports, generatedAt = new Date().toISOString(), invalidatedStates = []) {
  ...
  const rows = entries.map((entry) => {
    ...
    return {
      source: entry.source,
      hostname: report?.hostname || entry.hostname,
      reporter_name: report?.reporter_name || entry.reporter_name,
  ...
  const activeOrphanedRows = orphanedRows.filter((row) => !isArchivedInvalidatedRow(row, generatedAt));
  const archivedOrphanedRows = orphanedRows.filter((row) => isArchivedInvalidatedRow(row, generatedAt));
  const activeRows = activeEntryRows.concat(activeOrphanedRows);
  const allArchivedRows = archivedInvalidatedRows.concat(archivedOrphanedRows);
  ...
  return {
    ...
    orphaned_count: activeOrphanedRows.length,
