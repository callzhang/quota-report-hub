...
export function mergeLatestReport(previous, incoming) {
  ...
  // A standing central-refresh rejection means the pooled refresh token was presented to the provider
  // and refused. A client reporting "ok" only establishes that ITS OWN local credential works — that
  // machine may be running a credential it never uploaded, which says nothing about the blob the pool
  // hands out. Letting such a report clear the rejection is what kept claude-qpt0311@uw.edu marked
  // healthy, and in rotation to borrowers, for a month after its pooled token expired: the worker
  // re-reported the rejection every ~13 min and a client overwrote it every time. Only proof about
  // the pooled blob lifts it; the account otherwise stays invalid until its owner re-uploads.
  if (isCentralRefreshRejection(previous) && !clearsCentralRefreshRejection(incoming) && !isHardInvalidation(incoming)) {
    return {
      ...incoming,
      status: previous.status,
      error: previous.error,
      usage_summary: {
        ...(incoming.usage_summary || {}),
