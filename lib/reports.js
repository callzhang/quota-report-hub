...
export function mergeLatestReport(previous, incoming) {
  ...
  return {
    ...
    usage_summary: {
      ...
      // re-attach the evidence so the NEXT merge can still see it, whatever shape that report takes
...
// An ineligible auth passes through three stages, measured from when it went bad:
// visible in the active list, then Archived, then gone from the dashboard entirely.
const ARCHIVE_INVALIDATED_AFTER_MS = 48 * 60 * 60 * 1000;
const DROP_INVALIDATED_AFTER_MS = 14 * 24 * 60 * 60 * 1000;

// How long this row has been ineligible, or null when it is fine or nothing dates it.
// first_invalidated_at is the real clock. reported_at is the only fallback, for a row whose
// invalidation predates its notification record.
function ineligibleForMs(row, generatedAt) {
  ...
  const ineligibleSinceMs = Number.isFinite(firstInvalidatedMs)
    ? firstInvalidatedMs
    : Date.parse(row.reported_at || "");
  if (!Number.isFinite(ineligibleSinceMs)) {
    return null;
  }

  return generatedAtMs - ineligibleSinceMs;
}
...
export function authPoolStatusPayload(entries, reports, generatedAt = new Date().toISOString(), invalidatedStates = []) {
  ...
  // Three buckets, one pass. Past DROP_INVALIDATED_AFTER_MS a row lands in neither: an auth that
  // has been dead a fortnight is not news, and keeping it in Archived buries the ones that are.
  // It must be dropped from BOTH lists -- excluding it only from Archived would hand it straight
  // back to the active list, which is the opposite of retiring it.
  const activeEntryRows = [];
  const archivedInvalidatedRows = [];
  for (const row of rows) {
    const ineligibleMs = ineligibleForMs(row, generatedAt);
    if (ineligibleMs === null || ineligibleMs <= ARCHIVE_INVALIDATED_AFTER_MS) {
      activeEntryRows.push(row);
    } else if (ineligibleMs <= DROP_INVALIDATED_AFTER_MS) {
      archivedInvalidatedRows.push(row);
    }
  }
