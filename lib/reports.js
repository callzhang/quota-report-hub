...
// How long this row has been dead, or null while it is not. A Free plan is dead to the pool from
// its last report. Otherwise "dead" means BOTH tokens are gone: the refresh token rejected (or the
// invalidation record open) AND the access token unusable. A dead refresh token with a live access
// token is not dead -- it is a warning with a deadline, and the row stays in the active list.
// The clock starts when the second of the two went, so the 48h of visibility is real.
