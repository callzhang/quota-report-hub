...
function deriveDisplayWindow(window, report, generatedAt, windowName) {
  ...
  const generatedAtMs = timestampMs(generatedAt);
  // A reading older than the window it describes has been overtaken by at least one whole window,
  // so it is expired in exactly the sense below -- the difference is only that its own reset_at
  // never said so. Same reason string: it is the same fact, and decisions drop it for the same
  // reason (windowEvidenceLive, used by selection and availability alike).
  const evidenceExpired = !windowEvidenceLive(window, windowName, generatedAtMs);
  const resetExpired = evidenceExpired || (resetAtMs !== null && generatedAtMs !== null && resetAtMs <= generatedAtMs);
