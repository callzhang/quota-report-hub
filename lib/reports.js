...
export function sanitizeReport(input) {
  ...
  const rawFiveHour = sanitizeWindow(input.windows?.["5h"], 300);
  const rawOneWeek = sanitizeWindow(input.windows?.["1week"], 10080);
...
function authExpiryFields(row, generatedAt) {
  ...
  return {
    auth_expired: seconds <= 0,
    auth_expires_in_seconds: seconds,
  };
}
...
function deriveDisplayWindow(window, report, generatedAt) {
  ...
  } else if (report.status === "error" && resetExpired) {
    resetUnavailableReason = "stale_error_probe";
  }
...
function isRowIneligible(row) {
  return (
    ...
    row.error === "claude auth invalid (authentication_error)"
  );
}
