...
// A raw refresh token must never leave the encrypted auth blob. The lease needs to distinguish
// generations nevertheless, so use a domain-separated HMAC under the pool encryption key rather
// than a reversible value or an unhashed RT digest that could aid offline guessing.
export function refreshTokenFingerprint(source, refreshToken) {
  if (!refreshToken || !String(refreshToken).trim()) return null;
  const fingerprintKey = createHmac("sha256", authPoolKeyMaterial())
    .update("quota-report-hub/auth-refresh-fingerprint/v1", "utf8")
    .digest();
  return createHmac("sha256", fingerprintKey)
    .update(`${String(source)}\u0000${String(refreshToken)}`, "utf8")
    .digest("hex");
}

// The provider's own word for WHY a refresh token was refused, from the error body: OpenAI answers
// {"error": {"code": "refresh_token_expired" | "refresh_token_reused" | "refresh_token_invalidated" ...}}
// (Codex's own client classifies on exactly these), and other providers answer a bare string in
// `error`. Everything upstream of this used to see only "refresh http 401", which cannot tell an
// expired token (nobody used it for too long) from a reused one (someone else spent it) from a
// revoked one (the session was ended) -- three different explanations for the same dead grant, and
// the one question the pool's deaths kept coming back to. A code is a short identifier, never a
// token: restricted to [a-z0-9_.-] and 80 characters so nothing else can ride along in it.
export function providerRefreshErrorCode(payload) {
...
async function postRefresh(url, headers, body, fetchImpl) {
  ...
  if (!response.ok) {
    ...
    try {
      ...
      // A gateway page or an empty body has no code to read; the status still stands.
    }
    // 400/401 = the refresh token is dead (needs owner re-login); other codes are transient.
    return {
      ok: false,
      auth_rejected: [400, 401].includes(response.status),
      status: response.status,
      error: `refresh http ${response.status}`,
      provider_error_code: providerRefreshErrorCode(errorPayload),
    };
  }
  let payload;
  try {
...
function logRefreshOutcome(source, requestedScope, result, attempt) {
  try {
    console.log(JSON.stringify({
      ...
      provider_error_code: result.ok ? null : result.provider_error_code ?? null,
    }));
  } catch {
...
// Read-only usability check for a claude access token. Costs nothing and consumes nothing -- the
// opposite of proving a credential by spending its refresh token.
//
// Usable means usable for inference, because inference is the only thing a pooled claude credential
// is for. /api/oauth/profile and /api/oauth/usage accept a token that carries profile scope alone,
// and on 2026-09-17 the pooled claude-leizhang0121 token passed both for days while every borrower's
// inference call got 403 "OAuth token does not meet scope requirement". count_tokens sits behind the
// same scope gate as /v1/messages and is not metered, so it answers the question that matters without
// spending quota.
//
// Why a live token is enough on upload: a refresh REVOKES the access tokens already issued for the
// grant (§3.5), so an access token that still answers is itself evidence that nobody has refreshed
// the grant since it was minted -- which means the refresh token beside it has not been spent either.
...
const CLAUDE_INFERENCE_PROBE_URL = "https://api.anthropic.com/v1/messages/count_tokens";
const CLAUDE_INFERENCE_PROBE_MODEL = "claude-haiku-4-5-20251001";
...
export async function probeClaudeAccessToken(authJson, fetchImpl = fetch) {
  ...
  let response;
  try {
    response = await fetchImpl(CLAUDE_INFERENCE_PROBE_URL, {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({ model: CLAUDE_INFERENCE_PROBE_MODEL, messages: [{ role: "user", content: "ok" }] }),
    });
  } catch (error) {
    return { ok: false, status: null, rejected: false, lacks_inference: false, reason: String(error?.message || error).slice(0, 120) };
  }
  // The scope gate answers with a typed permission_error. Any other 403 (a proxy page, an org policy)
  // says nothing about the token's scope and is treated like any other bad day upstream.
  let errorType = null;
  if (response.status === 403) {
    try {
      errorType = (await response.json())?.error?.type ?? null;
    } catch {
      errorType = null;
    }
  }
  return {
    ok: response.ok,
    status: response.status,
    // 401 is the only answer that means "this credential is dead"; a 5xx or a rate limit is the
    // network having a bad day and must not be read as a dead account.
    rejected: response.status === 401,
    lacks_inference: errorType === "permission_error",
  };
}

export function refreshTokenFromAuthBlob(authJson, source) {
  let parsed;
  try {
    parsed = JSON.parse(authJson);
  } catch {
...
export async function verifyAndRefreshAuthBlob(authJson, source, fetchImpl = fetch) {
  ...
  if (!refreshed.ok) {
    return {
      ...
      provider_error_code: refreshed.provider_error_code ?? null,
    };
  }
  return {
    ok: true,
