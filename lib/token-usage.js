...
const BATCH_REQUIRED_KEYS = new Set(["installation_id", "batch_id", "rows"]);
...
const BATCH_ALLOWED_KEYS = new Set([...BATCH_REQUIRED_KEYS, "client_version", "replace_from"]);
...
function normalizeRow(input, { nowMs }) {
  ...
  if (row.total_tokens >= TOKEN_USAGE_IMPOSSIBLE_BUCKET_TOKENS) {
    throw new TokenUsageValidationError("total_tokens is implausibly large for a 15-minute bucket");
  }

  // One meaning for every provider: input is every input token with cache read and write as
  // subsets of it, and total is input plus output. The collector puts Claude's counters into this
  // shape at the source (Anthropic's own input_tokens excludes the cache), so a row that fails here
  // is malformed, not merely from the other provider.
