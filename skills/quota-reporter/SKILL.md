...
- if the server returns a Codex `repair_auth`, the guard reports `owner_relogin_required` but never installs that auth or changes the current account; repairing an owned invalidated Codex auth requires an explicit login, while only `replacement` may rotate Codex automatically
