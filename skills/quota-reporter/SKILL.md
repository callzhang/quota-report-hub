...
- if the authenticated caller has an invalidated Codex contribution, the server returns it as `repair_auth` before considering any healthy shared replacement; the guard installs that owner handback once, records `repair_auth_from_auth_pool`, and holds it at `owner_relogin_required` without re-uploading or fetching another account until an explicit login changes the credential digest
