...
test("pool health snapshots record and read back in chronological order", async () => {
  ...
  try {
    ...
    await mod.recordPoolHealthSnapshot({ captured_at: "2026-06-12T00:15:00Z", source: "codex", total: 10, ok_count: 8, hard_dead_count: 2, other_err_count: 0, skipped_count: 4, central_refresh_attempted: 2, central_refresh_ok: 1, central_refresh_rejected: 1 });
    ...
    assert.equal(snaps[0].skipped_count, 0);
    assert.equal(snaps[1].skipped_count, 4);
