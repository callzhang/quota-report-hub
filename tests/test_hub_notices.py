...
class RepeatIntervalTest(unittest.TestCase):
    ...
    def test_honours_the_interval_the_hub_chose(self):
        ...
        sent = {"notices": [dict(notice(code="usage_reporting_required"), repeat_seconds=3600)]}
        with mock.patch.object(quota_guard, "show_desktop_notification", return_value=True) as shown:
            quota_guard.notify_hub_notices(sent, now=0.0, state_path=self.state_path)
            quota_guard.notify_hub_notices(sent, now=1800.0, state_path=self.state_path)   # 30 min
            quota_guard.notify_hub_notices(sent, now=3601.0, state_path=self.state_path)   # 60 min
        self.assertEqual(shown.call_count, 2, "the client follows the hub's interval, not its own")
